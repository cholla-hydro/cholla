...
def concat_2d_dataset(
    ...
    *,
    ...
    """Concatenate 2D HDF5 Cholla datasets.

    This takes the single files generated per process and concatenate them
    into a single, large file. This function concatenates a single output time
    and can be called multiple times, potentially in parallel, to concatenate
    ...
    for rank in range(num_files):
        ...
        source_file = h5py.File(
            build_source_path(proc_id=rank, nfile=output_number), "r"
        )

        # Loop through and copy datasets
        for dataset in datasets_to_copy:
            # Determine locations and shifts for writing
            (i0_start, i0_end, i1_start, i1_end), file_in_slice = (
                __write_bounds_2d_dataset(source_file, dataset)
            )
            ...
            destination_file[dataset][i0_start:i0_end, i1_start:i1_end] += source_file[
                dataset
            ]
...
def __write_bounds_2d_dataset(source_file: h5py.File, dataset: str) -> tuple:
    ...
    Returns:
        ...
        `output_file[dataset][return[0]:return[1], return[2]:return[3]]`
        followed by a bool to indicate if the file is
        in the slice if concatenating a slice
    """

    if "xzr" in dataset:
        return (
            source_file.attrs["nx_min"][0],
            source_file.attrs["nx_max"][0],
            source_file.attrs["nz_min"][0],
            source_file.attrs["nz_max"][0],
        ), True
...
if __name__ == "__main__":
    ...
    cli.add_argument(
        ...
        type=str,
        required=True,
        help='What kind of 2D dataset to concatnate. Options are "slice", "proj", and "rot_proj"',
    )
    cli.add_argument(
        "--disable-xy",
        default=True,
        action="store_false",
        help="Disables concating the XY datasets.",
    )
    cli.add_argument(
        "--disable-yz",
        default=True,
        action="store_false",
        help="Disables concating the YZ datasets.",
    )
    cli.add_argument(
        "--disable-xz",
        default=True,
        action="store_false",
        help="Disables concating the XZ datasets.",
    )
    args = cli.parse_args()
