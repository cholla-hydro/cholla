...
void systemTest::SystemTestRunner::runTest()
{
  ...
  if (_particleDataExists) {
    ...
    std::vector<std::string> particleNames =
        _findDataSetNames(_testParticlesFileVec[0]);
    auto iter =
        std::find(particleNames.begin(), particleNames.end(), "density");
    *iter = "particle_density";

    _testDataSetNames.insert(_testDataSetNames.end(), particleNames.begin(),
                             particleNames.end());
  }
  ...
  EXPECT_GE(_testDataSetNames.size(), _fiducialDataSetNames.size())
      << std::endl
      << "Warning: The test data has " << _testDataSetNames.size()
      << " datasets and the fiducial data has " << _fiducialDataSetNames.size()
      << " datasets" << std::endl
      << std::endl;
  ...
  for (auto dataSetName : _fiducialDataSetNames) {
    ...
    ASSERT_EQ(std::count(_testDataSetNames.begin(), _testDataSetNames.end(),
                         dataSetName),
              1)
    ...
    std::vector<std::string> particleIDs = {
        "particle_IDs", "pos_x", "pos_y", "pos_z", "vel_x", "vel_y", "vel_z"};
    ...
    ASSERT_EQ(fiducialData.size(), testData.size())
        << "The fiducial and test '" << dataSetName
    ...
    for (size_t i = 0; i < testDims[0]; i++) {
      for (size_t j = 0; j < testDims[1]; j++) {
        for (size_t k = 0; k < testDims[2]; k++) {
          size_t index =
              (i * testDims[1] * testDims[2]) + (j * testDims[2]) + k;

          // Check for equality and iff not equal return difference
          double absoluteDiff;
          int64_t ulpsDiff;
          bool areEqual = testingUtilities::nearlyEqualDbl(
              fiducialData.at(index), testData.at(index), absoluteDiff,
              ulpsDiff, _fixedEpsilon);
          ASSERT_TRUE(areEqual)
              << std::endl
              << "Difference in " << dataSetName << " dataset at [" << i << ","
              << j << "," << k << "]" << std::endl
              << "The fiducial value is:       " << fiducialData[index]
              << std::endl
...
// =============================================================================
void systemTest::SystemTestRunner::runL1ErrorTest(
    double const &maxAllowedL1Error, double const &maxAllowedError)
{
  /// Only run if this variable is set to `true`. Generally this and
  /// globalCompareSystemTestResults should only be used for large MPI / tests
  /// where the user wishes to separate the execution of cholla and the /
  ...
  if (_particleDataExists) {
    std::string errMessage =
        "Error: SystemTestRunner::runL1ErrorTest does not support particles";
    throw std::runtime_error(errMessage);
  }
  if (not _hydroDataExists) {
    std::string errMessage =
        "Error: SystemTestRunner::runL1ErrorTest requires hydro data";
    throw std::runtime_error(errMessage);
  }
  ...
  for (auto dataSetName : _fiducialDataSetNames) {
    ...
    // This is a field data set
    initialData =
        loadTestFieldData(dataSetName, initialDims, initialHydroFieldsFileVec);
    // Get fiducial data
    finalData =
        loadTestFieldData(dataSetName, finalDims, _testHydroFieldsFileVec);

    // Check that they're the same length
    ASSERT_EQ(initialData.size(), finalData.size())
        << "The initial and final '" << dataSetName
        << "' datasets are not the same length";
    ...
    L1Error *= (1. / static_cast<double>(initialDims[0] * initialDims[1] *
                                         initialDims[2]));
    L2Norm += L1Error * L1Error;

    // Perform the correctness check
    EXPECT_LT(L1Error, maxAllowedL1Error)
        << "the L1 error for the " << dataSetName
  ...
  EXPECT_LT(maxError, maxAllowedError)
      << "The maximum error has exceeded the allowed value";
}
// =============================================================================
...
void systemTest::SystemTestRunner::launchCholla()
{
  // Launch Cholla. Note that this dumps all console output to the console
  // log file as requested by the user.
  std::string const chollaRunCommand =
      globalMpiLauncher.getString() + " " + std::to_string(numMpiRanks) + " " +
      _chollaPath + " " + _chollaSettingsPath + " " + chollaLaunchParams + " " +
      "outdir=" + _outputDirectory + "/" + " >> " + _consoleOutputPath +
      " 2>&1 ";
  auto returnEcho = system(
      ("echo Launch Command: " + chollaRunCommand + " >> " + _consoleOutputPath)
          .c_str());
...
// =============================================================================
void systemTest::SystemTestRunner::setFiducialData(
    std::string const &fieldName, std::vector<double> const &dataVec)
{
  // First check if there's a fiducial data file
  if (_fiducialDataSets.count(fieldName) > 0) {
    std::string errMessage = "Error: Fiducial dataset for field '" + fieldName +
...
std::vector<double> systemTest::SystemTestRunner::generateConstantData(
    double const &value, size_t const &nx, size_t const &ny, size_t const &nz)
{
...
std::vector<double> systemTest::SystemTestRunner::generateSineData(
    ...
    double const &ky, double const &kz, double const &phase, size_t const &nx,
  ...
  for (size_t i = 0; i < nx; i++) {
    for (size_t j = 0; j < ny; j++) {
      for (size_t k = 0; k < nz; k++) {
        double value =
            offset + amplitude * std::sin(kx * i + ky * j + kz * k + phase);
    ...
    : _particleDataExists(particleData), _hydroDataExists(hydroData)
{
  ...
  _chollaPath = ::globalChollaRoot.getString() + "/bin/cholla." +
  ...
  if (useSettingsFile) {
    _chollaSettingsPath = ::globalChollaRoot.getString() +
                          "/src/system_tests/input_files/" + _fullTestFileName +
                          ".txt";
  ...
  if (useFiducialFile) {
    _fiducialFilePath = ::globalChollaRoot.getString() +
                        "/cholla-tests-data/system_tests/" + _fullTestFileName +
                        ".h5";
  ...
  _consoleOutputPath =
      _outputDirectory + "/" + _fullTestFileName + "_console.log";
  ...
  if (system(("mkdir --parents " + _outputDirectory).c_str()) != 0) {
    std::cerr << "Warning: Directory '" + _outputDirectory +
...
// =============================================================================
void systemTest::SystemTestRunner::_safeMove(
    std::string const &sourcePath, std::string const &destinationDirectory)
{
  // TODO C++17 std::filesystem does this better
  _checkFileExists(sourcePath);
  if (std::rename(sourcePath.c_str(),
                  (destinationDirectory + "/" + sourcePath).c_str()) < 0) {
    std::string errMessage = "Error: File '" + sourcePath +
...
void systemTest::SystemTestRunner::_checkNumTimeSteps()
{
  ...
  if (_hydroDataExists) {
    ...
    std::string errMessage =
        "Error: Both hydro and particle data are  turned off.";
    throw std::invalid_argument(errMessage);
  }
...
std::vector<double> systemTest::SystemTestRunner::loadTestFieldData(
    std::string dataSetName, std::vector<size_t> &testDims,
    std::vector<H5::H5File> file)
{
  ...
  for (size_t rank = 0; rank < numMpiRanks; rank++) {
    ...
    std::string dimsNameLocal =
        (dataSetName.find("magnetic") != std::string::npos)
            ? "magnetic_field_dims_local"
            : "dims_local";
    H5::Attribute dimsLocalAttr =
        file[rank].openAttribute(dimsNameLocal.c_str());
    ...
    std::string const &dataSetName)
{
  ...
  for (size_t rank = 0; rank < numMpiRanks; rank++) {
    // Open the dataset
    H5::DataSet const testDataSet =
        _testParticlesFileVec[rank].openDataSet(dataSetName);
    ...
    unsortedTestData.insert(unsortedTestData.end(), tempVector.begin(),
                            tempVector.end());
  }
    ...
    std::string const &dataSetName)
{
  if (_fiducialFileExists) {
    ...
    if (dataSetName == "particle_IDs") {
      ...
      std::sort(tempSortedIndices.begin(), tempSortedIndices.end(),
                [&](size_t A, size_t B) -> bool {
                  return unsortedFiducialData.at(A) <
                         unsortedFiducialData.at(B);
                });
    }
