...
class tMHDCalculateHLLDFluxesCUDA : public ::testing::Test
{
 protected:
   ...
   */
  std::vector<Real> computeFluxes(std::vector<Real> stateLeft, std::vector<Real> stateRight, Real const &gamma,
   ...
   * y-magnetic field, z-magnetic field.
   */
  std::vector<Real> primitive2Conserved(std::vector<Real> const &input, double const &gamma,
...
TEST_F(tMHDCalculateHLLDFluxesCUDA, BrioAndWuShockTubeCorrectInputExpectCorrectOutput)
{
  ...
  std::vector<Real> const  // | Density | X-Velocity | Y-Velocity | Z-Velocity |
      ...
      leftFastRareLeftSide =
          primitive2Conserved({0.978576, 0.038603, -0.011074, Vz, 0.957621, Bx, 0.970288, Bz}, gamma, primitiveScalar),
      leftFastRareRightSide =
          primitive2Conserved({0.671655, 0.647082, -0.238291, Vz, 0.451115, Bx, 0.578240, Bz}, gamma, primitiveScalar),
      compoundLeftSide =
          primitive2Conserved({0.814306, 0.506792, -0.911794, Vz, 0.706578, Bx, -0.108819, Bz}, gamma, primitiveScalar),
      compoundPeak =
          primitive2Conserved({0.765841, 0.523701, -1.383720, Vz, 0.624742, Bx, -0.400787, Bz}, gamma, primitiveScalar),
      compoundRightSide =
          primitive2Conserved({0.695211, 0.601089, -1.583720, Vz, 0.515237, Bx, -0.537027, Bz}, gamma, primitiveScalar),
      contactLeftSide =
          primitive2Conserved({0.680453, 0.598922, -1.584490, Vz, 0.515856, Bx, -0.533616, Bz}, gamma, primitiveScalar),
      contactRightSide =
          primitive2Conserved({0.231160, 0.599261, -1.584820, Vz, 0.516212, Bx, -0.533327, Bz}, gamma, primitiveScalar),
      slowShockLeftSide =
          primitive2Conserved({0.153125, 0.086170, -0.683303, Vz, 0.191168, Bx, -0.850815, Bz}, gamma, primitiveScalar),
      slowShockRightSide = primitive2Conserved({0.117046, -0.238196, -0.165561, Vz, 0.087684, Bx, -0.903407, Bz}, gamma,
                                               primitiveScalar),
      rightFastRareLeftSide  = primitive2Conserved({0.117358, -0.228756, -0.158845, Vz, 0.088148, Bx, -0.908335, Bz},
                                                   gamma, primitiveScalar),
      rightFastRareRightSide = primitive2Conserved({0.124894, -0.003132, -0.002074, Vz, 0.099830, Bx, -0.999018, Bz},
                                                   gamma, primitiveScalar),
  ...
  for (size_t direction = 0; direction < 3; direction++) {
    // Initial Condition Checks
    {
      ...
      std::vector<Real> const testFluxes = computeFluxes(leftICs, leftICs, gamma, direction);
    ...
    }
    {
      ...
      std::vector<Real> const scalarFlux{0, 0, 0};
      Real thermalEnergyFlux             = 0.0;
      std::vector<Real> const testFluxes = computeFluxes(rightICs, rightICs, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(leftICs, rightICs, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const scalarFlux{-0.22885355953447648, -0.46073027567244362, -0.6854281091039145};
      Real thermalEnergyFlux             = -0.20673357746080046;
      std::vector<Real> const testFluxes = computeFluxes(rightICs, leftICs, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    ...
    // Cross wave checks
    {
      ...
      std::vector<Real> const testFluxes = computeFluxes(leftFastRareLeftSide, leftFastRareRightSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(leftFastRareRightSide, leftFastRareLeftSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(compoundLeftSide, compoundRightSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(compoundRightSide, compoundLeftSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(compoundLeftSide, compoundPeak, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(compoundPeak, compoundLeftSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(compoundPeak, compoundRightSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(compoundRightSide, compoundPeak, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(contactLeftSide, contactRightSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(contactRightSide, contactLeftSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(slowShockLeftSide, slowShockRightSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(slowShockRightSide, slowShockLeftSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes =
          computeFluxes(rightFastRareLeftSide, rightFastRareRightSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes =
          computeFluxes(rightFastRareRightSide, rightFastRareLeftSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
  }
}
...
TEST_F(tMHDCalculateHLLDFluxesCUDA, DaiAndWoodwardShockTubeCorrectInputExpectCorrectOutput)
{
  ...
  for (size_t direction = 0; direction < 3; direction++) {
    ...
    // Cross wave checks
    {
      ...
      std::vector<Real> const testFluxes =
          computeFluxes(leftFastShockLeftSide, leftFastShockRightSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes =
          computeFluxes(leftFastShockRightSide, leftFastShockLeftSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(leftRotationLeftSide, leftRotationRightSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(leftRotationRightSide, leftRotationLeftSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes =
          computeFluxes(leftSlowShockLeftSide, leftSlowShockRightSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes =
          computeFluxes(leftSlowShockRightSide, leftSlowShockLeftSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
    ...
    }
    {
      ...
      std::vector<Real> const testFluxes =
          computeFluxes(rightSlowShockLeftSide, rightSlowShockRightSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes =
          computeFluxes(rightSlowShockRightSide, rightSlowShockLeftSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes =
          computeFluxes(rightRotationLeftSide, rightRotationRightSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes =
          computeFluxes(rightRotationRightSide, rightRotationLeftSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes =
          computeFluxes(rightFastShockLeftSide, rightFastShockRightSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes =
          computeFluxes(rightFastShockRightSide, rightFastShockLeftSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
  }
}
...
TEST_F(tMHDCalculateHLLDFluxesCUDA, RyuAndJones4dShockTubeCorrectInputExpectCorrectOutput)
{
  ...
  for (size_t direction = 0; direction < 3; direction++) {
    ...
    // Cross wave checks
    {
      ...
      std::vector<Real> const testFluxes = computeFluxes(hydroRareLeftSide, hydroRareRightSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(hydroRareRightSide, hydroRareLeftSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes =
          computeFluxes(switchOnSlowShockLeftSide, switchOnSlowShockRightSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes =
          computeFluxes(switchOnSlowShockRightSide, switchOnSlowShockLeftSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
    ...
    }
    {
      ...
      std::vector<Real> const testFluxes = computeFluxes(rotationLeftSide, rotationRightSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(rotationRightSide, rotationLeftSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(fastRareLeftSide, fastRareRightSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(fastRareRightSide, fastRareLeftSide, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
  }
}
...
TEST_F(tMHDCalculateHLLDFluxesCUDA, EinfeldtStrongRarefactionCorrectInputExpectCorrectOutput)
{
  ...
  std::vector<Real> const  // | Density | X-Velocity | Y-Velocity | Z-Velocity |
      ...
      leftRarefactionCenter =
          primitive2Conserved({0.368580, -1.180830, Vy, Vz, 0.111253, Bx, 0.183044, Bz}, gamma, primitiveScalar),
      leftVxTurnOver =
          primitive2Conserved({0.058814, -0.125475, Vy, Vz, 0.008819, Bx, 0.029215, Bz}, gamma, primitiveScalar),
      midPoint = primitive2Conserved({0.034658, 0.000778, Vy, Vz, 0.006776, Bx, 0.017333, Bz}, gamma, primitiveScalar),
      rightVxTurnOver =
          primitive2Conserved({0.062587, 0.152160, Vy, Vz, 0.009521, Bx, 0.031576, Bz}, gamma, primitiveScalar),
      rightRarefactionCenter =
          primitive2Conserved({0.316485, 1.073560, Vy, Vz, 0.089875, Bx, 0.159366, Bz}, gamma, primitiveScalar),
  ...
  for (size_t direction = 0; direction < 3; direction++) {
    ...
    // Intermediate state checks
    {
      ...
      std::vector<Real> const testFluxes = computeFluxes(leftICs, leftRarefactionCenter, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const scalarFlux{-2.2139950592000002, -4.4572370036000004, -6.6310283749999996};
      Real thermalEnergyFlux             = -1.3499999999999996;
      std::vector<Real> const testFluxes = computeFluxes(leftRarefactionCenter, leftICs, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(leftRarefactionCenter, leftVxTurnOver, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(leftVxTurnOver, leftRarefactionCenter, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(leftVxTurnOver, midPoint, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(midPoint, leftVxTurnOver, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(midPoint, rightVxTurnOver, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(rightVxTurnOver, midPoint, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(rightVxTurnOver, rightRarefactionCenter, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(rightRarefactionCenter, rightVxTurnOver, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(rightRarefactionCenter, rightICs, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const scalarFlux{2.2139950592000002, 4.4572370036000004, 6.6310283749999996};
      Real thermalEnergyFlux             = 1.3499999999999996;
      std::vector<Real> const testFluxes = computeFluxes(rightICs, rightRarefactionCenter, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
  }
}
  ...
  for (size_t direction = 0; direction < 3; direction++) {
    {
      ...
      std::vector<Real> const testFluxes = computeFluxes(zeroMagneticField, zeroMagneticField, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
...
TEST_F(tMHDCalculateHLLDFluxesCUDA, DegenerateStateCorrectInputExpectCorrectOutput)
{
  ...
  // though
  for (size_t direction = 0; direction < 3; direction++) {
    std::vector<Real> const testFluxes = computeFluxes(state, state, gamma, direction);
    checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
  }
}
// =========================================================================
  ...
  for (size_t direction = 0; direction < 3; direction++) {
    {
      ...
      std::vector<Real> const scalarFlux{1.1069975296000001, 2.2286185018000002, 3.3155141874999998};
      Real thermalEnergyFlux             = -1.5;
      std::vector<Real> const testFluxes = computeFluxes(negativePressure, negativePressure, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(negativeEnergy, negativeEnergy, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      std::vector<Real> const testFluxes = computeFluxes(negativeDensity, negativeDensity, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      Real thermalEnergyFlux = 1.5000000000000001e+40;
      std::vector<Real> const testFluxes =
          computeFluxes(negativeDensityEnergyPressure, negativeDensityEnergyPressure, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
    {
      std::string const outputString{
      ...
      Real thermalEnergyFlux = -1.5000000000000001e+40;
      std::vector<Real> const testFluxes =
          computeFluxes(negativeDensityPressure, negativeDensityPressure, gamma, direction);
      checkResults(fiducialFlux, scalarFlux, thermalEnergyFlux, testFluxes, outputString, direction);
    }
  }
}
...
TEST(tMHDHlldInternalComputeStarState, CorrectInputNonDegenerateExpectCorrectOutput)
{
  ...
  std::vector<mhd::_internal::StarState> fiducialStarState{
      {24.101290139122913, 1.4626377138501221, 5.7559806612277464, 1023.8840191068896, 18.648382121236992,
       70.095850905078336},
...
TEST(tMHDHlldInternalStarFluxes, CorrectInputNonDegenerateExpectCorrectOutput)
{
  ...
  std::vector<mhd::_internal::Flux> fiducialFlux{
      {-45.270724071132321, 1369.1771532285088, -556.91765728768155, -2368.4452742393819, -21413.063415617493,
       -83.294404848633206, -504.8413875424834},
      {61.395380340435793, 283.48596932136809, -101.75517013858293, -51.343648925162142, -1413.4750762739586,
...
TEST(tMHDHlldInternalDoubleStarFluxes, CorrectInputExpectCorrectOutput)
{
  ...
  std::vector<mhd::_internal::Flux> const fiducialFlux{
      ...
      {10.040447333773216, 284.85426012223729, -487.87930516727664, 490.91728596722157, 59.061079503595323,
