                                           ...
                                           int const nz, int const n_cells, Real const gamma, int const n_fields)
{
  ...
  if (not reconstruction::Riemann_Thread_Guard<reconstruction>(nx, ny, nz, xid, yid, zid)) {
    ...
    // otherwise compute subsonic flux
      ...
      // & 36)
      Sm =
          (right_state.density * right_state.velocity.x() * (Sr - right_state.velocity.x()) -
      ...
      Els  = (left_state.energy * (Sl - left_state.velocity.x()) - left_state.pressure * left_state.velocity.x() +
             ps * Sm) /
            (Sl - Sm);
#ifdef DE
