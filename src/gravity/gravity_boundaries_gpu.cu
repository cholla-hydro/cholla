  ...
  #if defined(GRAV_ISOLATED_BOUNDARY_X) || \
      defined(GRAV_ISOLATED_BOUNDARY_Y) || defined(GRAV_ISOLATED_BOUNDARY_Z)

void __global__ Set_Potential_Boundaries_Isolated_kernel(
    int direction, int side, int size_buffer, int n_i, int n_j, int nx, int ny,
    int nz, int n_ghost, Real *potential_d, Real *pot_boundary_d)
{
  ...
  if (direction == 0) {
    ...
    if (side == 1)
      tid_pot = (nx - n_ghost + tid_k) + (tid_i + n_ghost) * nx +
                (tid_j + n_ghost) * nx * ny;
  }
  if (direction == 1) {
    if (side == 0)
    ...
    if (side == 1)
      tid_pot = (tid_i + n_ghost) + (ny - n_ghost + tid_k) * nx +
                (tid_j + n_ghost) * nx * ny;
  }
  if (direction == 2) {
    if (side == 0)
    ...
    if (side == 1)
      tid_pot = (tid_i + n_ghost) + (tid_j + n_ghost) * nx +
                (nz - n_ghost + tid_k) * nx * ny;
  }

  potential_d[tid_pot] = pot_boundary_d[tid_buffer];
}

void Grid3D::Set_Potential_Boundaries_Isolated_GPU(int direction, int side,
                                                   int *flags)
{
  ...
  cudaMemcpy(pot_boundary_d, pot_boundary_h, size_buffer * sizeof(Real),
             cudaMemcpyHostToDevice);
  cudaDeviceSynchronize();

  // Copy the potential boundary from buffer to potential array
  hipLaunchKernelGGL(Set_Potential_Boundaries_Isolated_kernel, dim1dGrid,
...
void __global__ Set_Potential_Boundaries_Periodic_kernel(
    int direction, int side, int n_i, int n_j, int nx, int ny, int nz,
    int n_ghost, Real *potential_d)
{
  // get a global thread ID
...
void Grid3D::Set_Potential_Boundaries_Periodic_GPU(int direction, int side,
                                                   int *flags)
{
  ...
  // Copy the potential boundary from buffer to potential array
  hipLaunchKernelGGL(Set_Potential_Boundaries_Periodic_kernel, dim1dGrid,
                     dim1dBlock, 0, 0, direction, side, n_i, n_j, nx_g, ny_g,
                     nz_g, n_ghost, Grav.F.potential_d);
}

__global__ void Load_Transfer_Buffer_GPU_kernel(
    int direction, int side, int size_buffer, int n_i, int n_j, int nx, int ny,
...
int Grid3D::Load_Gravity_Potential_To_Buffer_GPU(int direction, int side,
                                                 Real *buffer, int buffer_start)
{
  // printf( "Loading Gravity Buffer: Dir %d  side: %d \n", direction, side );
  int nx_pot, ny_pot, nz_pot, size_buffer, n_ghost_potential, n_ghost_transfer,
      n_i, n_j, ngrid;
  ;
  ...
  hipLaunchKernelGGL(Load_Transfer_Buffer_GPU_kernel, dim1dGrid, dim1dBlock, 0,
                     0, direction, side, size_buffer, n_i, n_j, nx_pot, ny_pot,
                     nz_pot, n_ghost_transfer, n_ghost_potential, potential_d,
                     send_buffer_d);
...
__global__ void Unload_Transfer_Buffer_GPU_kernel(
    int direction, int side, int size_buffer, int n_i, int n_j, int nx, int ny,
    int nz, int n_ghost_transfer, int n_ghost_potential, Real *potential_d,
    Real *transfer_buffer_d)
{
...
void Grid3D::Unload_Gravity_Potential_from_Buffer_GPU(int direction, int side,
                                                      Real *buffer,
                                                      int buffer_start)
{
  // printf( "Loading Gravity Buffer: Dir %d  side: %d \n", direction, side );
  ...
  hipLaunchKernelGGL(Unload_Transfer_Buffer_GPU_kernel, dim1dGrid, dim1dBlock,
                     0, 0, direction, side, size_buffer, n_i, n_j, nx_pot,
                     ny_pot, nz_pot, n_ghost_transfer, n_ghost_potential,
                     potential_d, recv_buffer_d);
}
