...
void Potential_SOR_3D::Allocate_Array_GPU_Real(Real **array_dev,
                                               grav_int_t size)
{
...
void Potential_SOR_3D::Allocate_Array_GPU_bool(bool **array_dev,
                                               grav_int_t size)
{
...
__global__ void Copy_Input_Kernel(int n_cells, Real *input_d, Real *density_d,
                                  ...
                                  Real current_a)
{
  ...
  #ifdef COSMOLOGY
  density_d[tid] =
      4 * M_PI * Grav_Constant * (input_d[tid] - dens_avrg) / current_a;
  #else
...
void Potential_SOR_3D::Copy_Input(int n_cells, Real *input_d,
                                  Real *input_density_h, Real Grav_Constant,
                                  Real dens_avrg, Real current_a)
{
  cudaMemcpy(input_d, input_density_h, n_cells * sizeof(Real),
  ...
  hipLaunchKernelGGL(Copy_Input_Kernel, dim1dGrid, dim1dBlock, 0, 0,
                     n_cells_local, F.input_d, F.density_d, Grav_Constant,
                     dens_avrg, current_a);
}

void Grav3D::Copy_Isolated_Boundary_To_GPU_buffer(Real *isolated_boundary_h,
...
__global__ void Initialize_Potential_Kernel(Real init_val, Real *potential_d,
                                            Real *density_d, int nx, int ny,
                                            int nz, int n_ghost)
{
...
void Potential_SOR_3D::Initialize_Potential(int nx, int ny, int nz,
                                            ...
                                            Real *potential_d, Real *density_d)
{
  ...
  hipLaunchKernelGGL(Initialize_Potential_Kernel, dim3dGrid, dim3dBlock, 0, 0,
                     1, potential_d, density_d, nx, ny, nz, n_ghost_potential);
}

__global__ void Iteration_Step_SOR(int n_cells, Real *density_d,
                                   Real *potential_d, int nx, int ny, int nz,
                                   int n_ghost, Real dx, Real dy, Real dz,
                                   Real omega, int parity, Real epsilon,
                                   bool *converged_d)
{
  ...
  phi_new = (1 - omega) * phi_c +
            omega / 6 *
                (phi_l + phi_r + phi_d + phi_u + phi_b + phi_t - dx * dx * rho);
...
void Potential_SOR_3D::Poisson_iteration(int n_cells, int nx, int ny, int nz,
                                         int n_ghost_potential, Real dx,
                                         Real dy, Real dz, Real omega,
                                         Real epsilon, Real *density_d,
                                         ...
                                         bool *converged_d)
{
  ...
  // converged_d );
  hipLaunchKernelGGL(Iteration_Step_SOR, dim3dGrid_half, dim3dBlock, 0, 0,
                     n_cells, density_d, potential_d, nx, ny, nz,
                     n_ghost_potential, dx, dy, dz, omega, 0, epsilon,
                     converged_d);
  ...
  hipLaunchKernelGGL(Iteration_Step_SOR, dim3dGrid_half, dim3dBlock, 0, 0,
                     n_cells, density_d, potential_d, nx, ny, nz,
                     n_ghost_potential, dx, dy, dz, omega, 1, epsilon,
                     converged_d);

  cudaMemcpy(converged_h, converged_d, sizeof(bool), cudaMemcpyDeviceToHost);
}

void Potential_SOR_3D::Poisson_iteration_Patial_1(
    int n_cells, int nx, int ny, int nz, int n_ghost_potential, Real dx,
...
void Potential_SOR_3D::Poisson_iteration_Patial_2(
    int n_cells, int nx, int ny, int nz, int n_ghost_potential, Real dx,
    Real dy, Real dz, Real omega, Real epsilon, Real *density_d,
    Real *potential_d, bool *converged_h, bool *converged_d)
{
...
__global__ void Set_Isolated_Boundary_GPU_kernel(
    int direction, int side, int size_buffer, int n_i, int n_j, int n_ghost,
    int nx_pot, int ny_pot, int nz_pot, Real *potential_d, Real *boundary_d)
{
  ...
  if (tid_i < 0 || tid_i >= n_i || tid_j < 0 || tid_j >= n_j || tid_k < 0 ||
      tid_k >= n_ghost)
    return;
  ...
  if (direction == 0) {
    if (side == 0)
      tid_pot = (tid_k) + (tid_i + n_ghost) * nx_pot +
                (tid_j + n_ghost) * nx_pot * ny_pot;
    if (side == 1)
      tid_pot = (tid_k + nx_local + n_ghost) + (tid_i + n_ghost) * nx_pot +
                (tid_j + n_ghost) * nx_pot * ny_pot;
  }
  if (direction == 1) {
    if (side == 0)
      tid_pot = (tid_i + n_ghost) + (tid_k)*nx_pot +
                (tid_j + n_ghost) * nx_pot * ny_pot;
    if (side == 1)
      tid_pot = (tid_i + n_ghost) + (tid_k + ny_local + n_ghost) * nx_pot +
                (tid_j + n_ghost) * nx_pot * ny_pot;
  }
  if (direction == 2) {
    if (side == 0)
      tid_pot = (tid_i + n_ghost) + (tid_j + n_ghost) * nx_pot +
                (tid_k)*nx_pot * ny_pot;
    if (side == 1)
      tid_pot = (tid_i + n_ghost) + (tid_j + n_ghost) * nx_pot +
                (tid_k + nz_local + n_ghost) * nx_pot * ny_pot;
  }

  potential_d[tid_pot] = boundary_d[tid_buffer];
}

void Potential_SOR_3D::Set_Isolated_Boundary_GPU(int direction, int side,
                                                 Real *boundary_d)
{
  // #ifdef MPI_CHOLLA
  ...
  hipLaunchKernelGGL(Set_Isolated_Boundary_GPU_kernel, dim1dGrid, dim1dBlock, 0,
                     0, direction, side, size_buffer, n_i, n_j, n_ghost, nx_pot,
                     ny_pot, nz_pot, F.potential_d, boundary_d);
}

void Potential_SOR_3D::Copy_Output(Real *output_potential)
{
  cudaMemcpy(output_potential, F.potential_d, n_cells_potential * sizeof(Real),
             cudaMemcpyDeviceToHost);
}

void Potential_SOR_3D::Copy_Potential_From_Host(Real *output_potential)
{
  cudaMemcpy(F.potential_d, output_potential, n_cells_potential * sizeof(Real),
             cudaMemcpyHostToDevice);
}

__global__ void Load_Transfer_Buffer_GPU_kernel_SOR(
    int direction, int side, int size_buffer, int n_i, int n_j, int nx, int ny,
    int nz, int n_ghost_transfer, int n_ghost_potential, Real *potential_d,
    Real *transfer_buffer_d)
{
  ...
  if (direction == 0) {
    ...
    if (side == 1)
      tid_pot = (nx - n_ghost_potential - n_ghost_transfer + tid_k) +
                (tid_i)*nx + (tid_j)*nx * ny;
  }
  if (direction == 1) {
    if (side == 0)
    ...
    if (side == 1)
      tid_pot = (tid_i) +
                (ny - n_ghost_potential - n_ghost_transfer + tid_k) * nx +
                (tid_j)*nx * ny;
  }
  if (direction == 2) {
    if (side == 0)
    ...
    if (side == 1)
      tid_pot = (tid_i) + (tid_j)*nx +
                (nz - n_ghost_potential - n_ghost_transfer + tid_k) * nx * ny;
  }
  transfer_buffer_d[tid_buffer] = potential_d[tid_pot];
}

__global__ void Load_Transfer_Buffer_GPU_Half_kernel(
    int direction, int side, int size_buffer, int n_i, int n_j, int nx, int ny,
    int nz, int n_ghost_transfer, int n_ghost_potential, Real *potential_d,
    Real *transfer_buffer_d, int parity)
{
  ...
  if (direction == 0) {
    if (side == 0)
      tid_pot = (n_ghost_potential + tid_k) + (tid_i)*nx_pot +
                (tid_j)*nx_pot * ny_pot;
    if (side == 1)
      tid_pot = (nx_pot - n_ghost_potential - n_ghost_transfer + tid_k) +
                (tid_i)*nx_pot + (tid_j)*nx_pot * ny_pot;
  }
  if (direction == 1) {
    if (side == 0)
      tid_pot = (tid_i) + (n_ghost_potential + tid_k) * nx_pot +
                (tid_j)*nx_pot * ny_pot;
    if (side == 1)
      tid_pot =
          (tid_i) +
          (ny_pot - n_ghost_potential - n_ghost_transfer + tid_k) * nx_pot +
          (tid_j)*nx_pot * ny_pot;
  }
  if (direction == 2) {
    if (side == 0)
      tid_pot = (tid_i) + (tid_j)*nx_pot +
                (n_ghost_potential + tid_k) * nx_pot * ny_pot;
    if (side == 1)
      tid_pot = (tid_i) + (tid_j)*nx_pot +
                (nz_pot - n_ghost_potential - n_ghost_transfer + tid_k) *
                    nx_pot * ny_pot;
  }
...
__global__ void Unload_Transfer_Buffer_GPU_kernel_SOR(
    int direction, int side, int size_buffer, int n_i, int n_j, int nx, int ny,
    int nz, int n_ghost_transfer, int n_ghost_potential, Real *potential_d,
    Real *transfer_buffer_d)
{
  ...
  if (direction == 0) {
    if (side == 0)
      tid_pot = (n_ghost_potential - n_ghost_transfer + tid_k) + (tid_i)*nx +
                (tid_j)*nx * ny;
    if (side == 1)
  ...
  if (direction == 1) {
    if (side == 0)
      tid_pot = (tid_i) + (n_ghost_potential - n_ghost_transfer + tid_k) * nx +
                (tid_j)*nx * ny;
    if (side == 1)
      tid_pot =
          (tid_i) + (ny - n_ghost_potential + tid_k) * nx + (tid_j)*nx * ny;
  }
  if (direction == 2) {
    if (side == 0)
      tid_pot = (tid_i) + (tid_j)*nx +
                (n_ghost_potential - n_ghost_transfer + tid_k) * nx * ny;
    if (side == 1)
      tid_pot =
          (tid_i) + (tid_j)*nx + (nz - n_ghost_potential + tid_k) * nx * ny;
  }
    ...
    Real *transfer_buffer_d, int parity)
{
  ...
  if (direction == 0) {
    if (side == 0)
      tid_pot = (n_ghost_potential - n_ghost_transfer + tid_k) +
                (tid_i)*nx_pot + (tid_j)*nx_pot * ny_pot;
    if (side == 1)
      tid_pot = (nx_pot - n_ghost_potential + tid_k) + (tid_i)*nx_pot +
                (tid_j)*nx_pot * ny_pot;
  }
  if (direction == 1) {
    if (side == 0)
      tid_pot = (tid_i) +
                (n_ghost_potential - n_ghost_transfer + tid_k) * nx_pot +
                (tid_j)*nx_pot * ny_pot;
    if (side == 1)
      tid_pot = (tid_i) + (ny_pot - n_ghost_potential + tid_k) * nx_pot +
                (tid_j)*nx_pot * ny_pot;
  }
  if (direction == 2) {
    if (side == 0)
      tid_pot =
          (tid_i) + (tid_j)*nx_pot +
          (n_ghost_potential - n_ghost_transfer + tid_k) * nx_pot * ny_pot;
    if (side == 1)
      tid_pot = (tid_i) + (tid_j)*nx_pot +
                (nz_pot - n_ghost_potential + tid_k) * nx_pot * ny_pot;
  }
  potential_d[tid_pot] = transfer_buffer_d[tid_buffer];
}

void Potential_SOR_3D::Load_Transfer_Buffer_GPU(
    int direction, int side, int nx, int ny, int nz, int n_ghost_transfer,
    int n_ghost_potential, Real *potential_d, Real *transfer_buffer_d)
{
  ...
  hipLaunchKernelGGL(Load_Transfer_Buffer_GPU_kernel_SOR, dim1dGrid, dim1dBlock,
                     0, 0, direction, side, size_buffer, n_i, n_j, nx_pot,
                     ny_pot, nz_pot, n_ghost_transfer, n_ghost_potential,
                     potential_d, transfer_buffer_d);
}

void Potential_SOR_3D::Load_Transfer_Buffer_Half_GPU(
    int direction, int side, int nx, int ny, int nz, int n_ghost_transfer,
    int n_ghost_potential, Real *potential_d, Real *transfer_buffer_d)
{
  ...
  hipLaunchKernelGGL(Load_Transfer_Buffer_GPU_Half_kernel, dim1dGrid,
                     dim1dBlock, 0, 0, direction, side, size_buffer, n_i, n_j,
                     nx, ny, nz, n_ghost_transfer, n_ghost_potential,
                     potential_d, transfer_buffer_d, iteration_parity);
}

void Potential_SOR_3D::Unload_Transfer_Buffer_GPU(
    int direction, int side, int nx, int ny, int nz, int n_ghost_transfer,
    int n_ghost_potential, Real *potential_d, Real *transfer_buffer_d)
{
  int nx_pot, ny_pot, nz_pot, size_buffer, n_i, n_j, ngrid;
  ...
  // n_ghost_potential, potential_d, transfer_buffer_d  );
  hipLaunchKernelGGL(Unload_Transfer_Buffer_GPU_kernel_SOR, dim1dGrid,
                     dim1dBlock, 0, 0, direction, side, size_buffer, n_i, n_j,
                     nx_pot, ny_pot, nz_pot, n_ghost_transfer,
                     n_ghost_potential, potential_d, transfer_buffer_d);
}

void Potential_SOR_3D::Unload_Transfer_Buffer_Half_GPU(
    int direction, int side, int nx, int ny, int nz, int n_ghost_transfer,
    int n_ghost_potential, Real *potential_d, Real *transfer_buffer_d)
{
  int size_buffer, n_i, n_j, ngrid;
  ...
  // n_ghost_potential, potential_d, transfer_buffer_d, iteration_parity  );
  hipLaunchKernelGGL(Unload_Transfer_Buffer_GPU_Half_kernel, dim1dGrid,
                     dim1dBlock, 0, 0, direction, side, size_buffer, n_i, n_j,
                     nx, ny, nz, n_ghost_transfer, n_ghost_potential,
                     potential_d, transfer_buffer_d, iteration_parity);
}
void Potential_SOR_3D::Copy_Transfer_Buffer_To_Host(int size_buffer,
                                                    Real *transfer_buffer_h,
                                                    Real *transfer_buffer_d)
{
  CudaSafeCall(cudaMemcpy(transfer_buffer_h, transfer_buffer_d,
                          size_buffer * sizeof(Real), cudaMemcpyDeviceToHost));
}

void Potential_SOR_3D::Copy_Transfer_Buffer_To_Device(int size_buffer,
                                                      Real *transfer_buffer_h,
                                                      Real *transfer_buffer_d)
{
  CudaSafeCall(cudaMemcpy(transfer_buffer_d, transfer_buffer_h,
                          size_buffer * sizeof(Real), cudaMemcpyHostToDevice));
}
