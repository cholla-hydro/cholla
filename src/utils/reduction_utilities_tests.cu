...
TEST(tALLBackports, AtomicMinLL)
{
  ...
  long long reference = std::min_element(host_vals.begin(), host_vals.end());

  // construct a device vector that holds copies of each host value
  cuda_utilities::DeviceVector<Real> device_vals(device_vals.size());
  device_vals.cpyHostToDevice(host_vals);

  // construct an output buffer where we will write the results
  cuda_utilities::DeviceVector<Real> device_outbuffer(1);
  ...
  ASSERT_EQ(reference, device_outbuffer[0]) << "reduction_utilities::backport::atomicMin produced an unexpected result";
}
