...
__global__ void Check_For_Extreme_Temperature_Kernel(Real* dev_conserved, int n_cells, Real gamma, Real lower_limit,
                                                     Real upper_limit, int marker, bool* out_bool)
{
  ...
  */
  ...
  if (T_kelvin <= lower_limit || T_kelvin >= upper_limit) {
    out_bool[0] = true;
    kernel_printf(
        "Check_For_Extreme_Temperature_Kernel found Value: %g E: %g E_thermal: %g E_kinetic: %g Marker: %d Thread: "
        "%d\n",
        T_kelvin, E, E_thermal, E_kinetic, marker, id);
  }
}

void Check_For_Extreme_Temperature(Real* dev_conserved, int n_cells, Real gamma, Real lower_limit, Real upper_limit,
                                   int marker)
{
  ...
  hipLaunchKernelGGL(Check_For_Extreme_Temperature_Kernel, dim1dGrid, dim1dBlock, 0, 0, dev_conserved, n_cells, gamma,
                     lower_limit, upper_limit, marker, out_bool);
