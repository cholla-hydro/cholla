 ...
 */
inline __host__ __device__ Real _magnetosonicSpeed(
    Real const &density, Real const &gasPressure, Real const &magneticX,
    ...
    Real const &waveChoice)
{
  ...
  Real bSquared =
      bXSquared + ((magneticY * magneticY) + (magneticZ * magneticZ));
  ...
  return sqrt((term1 + waveChoice * term2) /
              (2.0 * fmax(density, TINY_NUMBER)));
}
// =====================================================================
}  // namespace _internal
 ...
 */
inline __host__ __device__ Real computeEnergy(
    Real const &pressure, Real const &density, Real const &velocityX,
...
#ifdef MHD
  energy += 0.5 * (magneticX * magneticX +
 ...
 */
inline __host__ __device__ Real computeGasPressure(
    Real const &energy, Real const &density, Real const &momentumX,
    ...
    Real const &magneticY, Real const &magneticZ, Real const &gamma)
{
  Real pressure = (gamma - 1.) *
                  (energy -
                   0.5 *
                       (momentumX * momentumX +
                        ((momentumY * momentumY) + (momentumZ * momentumZ))) /
                       density -
                   0.5 * (magneticX * magneticX +
                          ((magneticY * magneticY) + (magneticZ * magneticZ))));
 ...
 */
inline __host__ __device__ Real computeThermalEnergy(
    Real const &energyTot, Real const &density, Real const &momentumX,
    Real const &momentumY, Real const &momentumZ, Real const &magneticX,
    Real const &magneticY, Real const &magneticZ, Real const &gamma)
{
  return energyTot -
...
{
  return 0.5 * (magneticX * magneticX +
                ((magneticY * magneticY) + (magneticZ * magneticZ)));
}
// =========================================================================
...
{
  Real pTot =
      gasPressure + 0.5 * (magneticX * magneticX +
                           ((magneticY * magneticY) + (magneticZ * magneticZ)));
 ...
 */
inline __host__ __device__ Real fastMagnetosonicSpeed(
    Real const &density, Real const &pressure, Real const &magneticX,
    Real const &magneticY, Real const &magneticZ, Real const &gamma)
{
  // Compute the sound speed
  return mhd::utils::_internal::_magnetosonicSpeed(
      density, pressure, magneticX, magneticY, magneticZ, gamma, 1.0);
}
// =========================================================================
 ...
 */
inline __host__ __device__ Real slowMagnetosonicSpeed(
    Real const &density, Real const &pressure, Real const &magneticX,
    Real const &magneticY, Real const &magneticZ, Real const &gamma)
{
  // Compute the sound speed
 ...
 */
inline __host__ __device__ auto cellCenteredMagneticFields(
    Real const *dev_conserved, size_t const &id, size_t const &xid,
    size_t const &yid, size_t const &zid, size_t const &n_cells,
    size_t const &nx, size_t const &ny)
{
  ...
  Real avgBx =
      (xid > 0)
          ?
          /*if true*/ 0.5 *
              (dev_conserved[(grid_enum::magnetic_x)*n_cells + id] +
               dev_conserved[(grid_enum::magnetic_x)*n_cells +
                             cuda_utilities::compute1DIndex(xid - 1, yid, zid,
                                                            nx, ny)])
          :
  ...
  Real avgBy =
      (yid > 0)
          ?
          /*if true*/ 0.5 *
              (dev_conserved[(grid_enum::magnetic_y)*n_cells + id] +
               dev_conserved[(grid_enum::magnetic_y)*n_cells +
                             cuda_utilities::compute1DIndex(xid, yid - 1, zid,
                                                            nx, ny)])
          :
  ...
  Real avgBz =
      (zid > 0)
          ?
          /*if true*/ 0.5 *
              (dev_conserved[(grid_enum::magnetic_z)*n_cells + id] +
               dev_conserved[(grid_enum::magnetic_z)*n_cells +
                             cuda_utilities::compute1DIndex(xid, yid, zid - 1,
                                                            nx, ny)])
          :
