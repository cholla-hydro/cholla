    ...
    Real const dx, Real const dy, Real const dz)
{
  ...
  if (xid < nx - 2 and yid < ny - 2 and zid < nz - 2) {
    ...
    // updates
    Real electric_x_1 = ctElectricFields[(
        cuda_utilities::compute1DIndex(xid, yid + 1, zid, nx, ny))];
    Real electric_x_2 = ctElectricFields[(
        cuda_utilities::compute1DIndex(xid, yid, zid + 1, nx, ny))];
    Real electric_x_3 = ctElectricFields[(
        cuda_utilities::compute1DIndex(xid, yid + 1, zid + 1, nx, ny))];
