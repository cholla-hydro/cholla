 ...
 *  \brief When passed the left and right boundary values at an interface,
 ...
 Sanders et al, 1998. */
__global__ void calc_eta_x_3D(Real *dev_bounds_L, Real *dev_bounds_R,
                              Real *eta_x, int nx, int ny, int nz, int n_ghost,
                              Real gamma)
{
  ...
  if (xid > n_ghost - 2 && xid < nx - n_ghost && yid > n_ghost - 2 &&
      yid < ny - n_ghost + 1 && zid > n_ghost - 2 && zid < nz - n_ghost + 1) {
 ...
 *  \brief When passed the left and right boundary values at an interface,
 calculates the eta value for the interface according to the forumulation in
 Sanders et al, 1998. */
__global__ void calc_eta_y_3D(Real *dev_bounds_L, Real *dev_bounds_R,
                              Real *eta_y, int nx, int ny, int nz, int n_ghost,
                              Real gamma)
{
  int n_cells = nx * ny * nz;
  ...
  if (yid > n_ghost - 2 && yid < ny - n_ghost && xid > n_ghost - 2 &&
      xid < nx - n_ghost + 1 && zid > n_ghost - 2 && zid < nz - n_ghost + 1) {
    // load the interface values into registers
    id = xid + yid * nx + zid * nx * ny;
 ...
 *  \brief When passed the left and right boundary values at an interface,
 calculates the eta value for the interface according to the forumulation in
 Sanders et al, 1998. */
__global__ void calc_eta_z_3D(Real *dev_bounds_L, Real *dev_bounds_R,
                              Real *eta_z, int nx, int ny, int nz, int n_ghost,
                              Real gamma)
{
  int n_cells = nx * ny * nz;
  ...
  // z-direction
  if (zid > n_ghost - 2 && zid < nz - n_ghost && xid > n_ghost - 2 &&
      xid < nx - n_ghost + 1 && yid > n_ghost - 2 && yid < ny - n_ghost + 1) {
    // load the interface values into registers
    id = xid + yid * nx + zid * nx * ny;
    pl =
        (dev_bounds_L[4 * n_cells + id] -
    ...
    eta_z[id] =
        0.5 * fabs((dev_bounds_R[3 * n_cells + id] / dev_bounds_R[id] + ar) -
 ...
 *  \brief When passed the eta values at every interface, calculates
 ...
 Sanders et al, 1998. */
__global__ void calc_etah_x_3D(Real *eta_x, Real *eta_y, Real *eta_z,
                               Real *etah_x, int nx, int ny, int nz,
                               int n_ghost)
{
  // get a thread ID
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
  ...
  if (xid > n_ghost - 2 && xid < nx - n_ghost && yid > n_ghost - 1 &&
    ...
    etah = fmax(eta_y[xid + (yid - 1) * nx + zid * nx * ny],
                eta_y[xid + 1 + (yid - 1) * nx + zid * nx * ny]);
 ...
 *  \brief When passed the eta values at every interface, calculates
           the eta_h value for the interface according to the forumulation in
 Sanders et al, 1998. */
__global__ void calc_etah_y_3D(Real *eta_x, Real *eta_y, Real *eta_z,
                               Real *etah_y, int nx, int ny, int nz,
                               int n_ghost)
{
  // get a thread ID
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
  ...
  if (yid > n_ghost - 2 && yid < ny - n_ghost && xid > n_ghost - 1 &&
      xid < nx - n_ghost && zid > n_ghost - 1 && zid < nz - n_ghost) {
    id = xid + yid * nx + zid * nx * ny;

    etah = fmax(eta_z[xid + yid * nx + (zid - 1) * nx * ny],
                eta_z[xid + (yid + 1) * nx + (zid - 1) * nx * ny]);
    etah = fmax(etah, eta_z[id]);
 ...
 *  \brief When passed the eta values at every interface, calculates
           the eta_h value for the interface according to the forumulation in
 Sanders et al, 1998. */
__global__ void calc_etah_z_3D(Real *eta_x, Real *eta_y, Real *eta_z,
                               Real *etah_z, int nx, int ny, int nz,
                               int n_ghost)
{
  // get a thread ID
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
  ...
  // z-direction
  if (zid > n_ghost - 2 && zid < nz - n_ghost && xid > n_ghost - 1 &&
      xid < nx - n_ghost && yid > n_ghost - 1 && yid < ny - n_ghost) {
    id = xid + yid * nx + zid * nx * ny;

    etah = fmax(eta_x[xid - 1 + yid * nx + zid * nx * ny],
                eta_x[xid - 1 + yid * nx + (zid + 1) * nx * ny]);
    etah = fmax(etah, eta_x[id]);
