...
__global__ void Update_Conserved_Variables_3D_FC(Real *dev_conserved, Real *dev_F1_x, Real *dev_F1_y,  Real *dev_F1_z,
                Real *dev_F2_x, Real *dev_F2_y, Real *dev_F2_z, int nx, int ny, int nz, int x_off, int y_off, int z_off,
                int n_ghost, Real dx, Real dy, Real dz, Real xbound, Real ybound, Real zbound, Real dt, Real gamma)
{
  int id, xid, yid, zid, n_cells;
  int imo, jmo, kmo;
  #if defined (DE) || defined(STATIC_GRAV)
  ...
  if (xid > n_ghost-1 && xid < nx-n_ghost && yid > n_ghost-1 && yid < ny-n_ghost && zid > n_ghost-1 && zid < nz-n_ghost)
  {
    ...
    // update the conserved variable array
    dev_conserved[            id] += dtodx * (dev_F2_x[            imo] - dev_F2_x[            id])
                                  +  dtody * (dev_F2_y[            jmo] - dev_F2_y[            id])
                                  +  dtodz * (dev_F2_z[            kmo] - dev_F2_z[            id]);
    dev_conserved[  n_cells + id] += dtodx * (dev_F2_x[  n_cells + imo] - dev_F2_x[  n_cells + id])
                                  +  dtody * (dev_F2_y[  n_cells + jmo] - dev_F2_y[  n_cells + id])
                                  +  dtodz * (dev_F2_z[  n_cells + kmo] - dev_F2_z[  n_cells + id]);
    dev_conserved[2*n_cells + id] += dtodx * (dev_F2_x[2*n_cells + imo] - dev_F2_x[2*n_cells + id])
                                  +  dtody * (dev_F2_y[2*n_cells + jmo] - dev_F2_y[2*n_cells + id])
                                  +  dtodz * (dev_F2_z[2*n_cells + kmo] - dev_F2_z[2*n_cells + id]);
    dev_conserved[3*n_cells + id] += dtodx * (dev_F2_x[3*n_cells + imo] - dev_F2_x[3*n_cells + id])
                                  +  dtody * (dev_F2_y[3*n_cells + jmo] - dev_F2_y[3*n_cells + id])
                                  +  dtodz * (dev_F2_z[3*n_cells + kmo] - dev_F2_z[3*n_cells + id]);
    dev_conserved[4*n_cells + id] += dtodx * (dev_F2_x[4*n_cells + imo] - dev_F2_x[4*n_cells + id])
                                  +  dtody * (dev_F2_y[4*n_cells + jmo] - dev_F2_y[4*n_cells + id])
                                  +  dtodz * (dev_F2_z[4*n_cells + kmo] - dev_F2_z[4*n_cells + id]);
    #ifdef DE
    dev_conserved[5*n_cells + id] += dtodx * (dev_F2_x[5*n_cells + imo] - dev_F2_x[5*n_cells + id])
                                  +  dtody * (dev_F2_y[5*n_cells + jmo] - dev_F2_y[5*n_cells + id])
                                  +  dtodz * (dev_F2_z[5*n_cells + kmo] - dev_F2_z[5*n_cells + id])
                                  +  0.5*P*(dtodx*(vx_imo-vx_ipo) + dtody*(vy_jmo-vy_jpo) + dtodz*(vz_kmo-vz_kpo));
    #endif
    #ifdef STATIC_GRAV 
    calc_g_3D_CUDA(xid, yid, zid, x_off, y_off, z_off, n_ghost, dx, dy, dz, xbound, ybound, zbound, &gx, &gy, &gz);
    ...
    P  = (dev_conserved[4*n_cells + id] - 0.5*d*(vx*vx + vy*vy + vz*vz)) * (gamma - 1.0);    
    if (d < 0 || d != d || P < 0 || P != P) {
    // redo the update with the first-order fluxes
    dev_conserved[            id] -= dtodx * (dev_F2_x[            imo] - dev_F2_x[            id])
                                  +  dtody * (dev_F2_y[            jmo] - dev_F2_y[            id])
                                  +  dtodz * (dev_F2_z[            kmo] - dev_F2_z[            id]);
    dev_conserved[  n_cells + id] -= dtodx * (dev_F2_x[  n_cells + imo] - dev_F2_x[  n_cells + id])
    ...
    dev_conserved[5*n_cells + id] -= dtodx * (dev_F2_x[5*n_cells + imo] - dev_F2_x[5*n_cells + id])
                                  ...
                                  +  0.5*P*(dtodx*(vx_imo-vx_ipo) + dtody*(vy_jmo-vy_jpo) + dtodz*(vz_kmo-vz_kpo));
    #endif
    dev_conserved[            id] += dtodx * (dev_F1_x[            imo] - dev_F1_x[            id])
                                  +  dtody * (dev_F1_y[            jmo] - dev_F1_y[            id])
                                  +  dtodz * (dev_F1_z[            kmo] - dev_F1_z[            id]);
    dev_conserved[  n_cells + id] += dtodx * (dev_F1_x[  n_cells + imo] - dev_F1_x[  n_cells + id])
                                  +  dtody * (dev_F1_y[  n_cells + jmo] - dev_F1_y[  n_cells + id])
                                  +  dtodz * (dev_F1_z[  n_cells + kmo] - dev_F1_z[  n_cells + id]);
    dev_conserved[2*n_cells + id] += dtodx * (dev_F1_x[2*n_cells + imo] - dev_F1_x[2*n_cells + id])
                                  +  dtody * (dev_F1_y[2*n_cells + jmo] - dev_F1_y[2*n_cells + id])
                                  +  dtodz * (dev_F1_z[2*n_cells + kmo] - dev_F1_z[2*n_cells + id]);
    dev_conserved[3*n_cells + id] += dtodx * (dev_F1_x[3*n_cells + imo] - dev_F1_x[3*n_cells + id])
                                  +  dtody * (dev_F1_y[3*n_cells + jmo] - dev_F1_y[3*n_cells + id])
                                  +  dtodz * (dev_F1_z[3*n_cells + kmo] - dev_F1_z[3*n_cells + id]);
    dev_conserved[4*n_cells + id] += dtodx * (dev_F1_x[4*n_cells + imo] - dev_F1_x[4*n_cells + id])
                                  +  dtody * (dev_F1_y[4*n_cells + jmo] - dev_F1_y[4*n_cells + id])
                                  +  dtodz * (dev_F1_z[4*n_cells + kmo] - dev_F1_z[4*n_cells + id]);
    #ifdef DE
    dev_conserved[5*n_cells + id] += dtodx * (dev_F1_x[5*n_cells + imo] - dev_F1_x[5*n_cells + id])
                                  +  dtody * (dev_F1_y[5*n_cells + jmo] - dev_F1_y[5*n_cells + id])
                                  +  dtodz * (dev_F1_z[5*n_cells + kmo] - dev_F1_z[5*n_cells + id])
                                  +  0.5*P*(dtodx*(vx_imo-vx_ipo) + dtody*(vy_jmo-vy_jpo) + dtodz*(vz_kmo-vz_kpo));
    #endif    
    }
  }
