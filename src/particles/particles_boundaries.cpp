...
void Grid3D::Wait_and_Unload_MPI_Comm_Particles_Buffers_BLOCK(int dir,
                                                              int *flags)
{
  int iwait;
  int index    = 0;
...
void Grid3D::Wait_NTransfer_and_Request_Recv_Particles_Transfer_BLOCK(
    int dir, int *flags)
{
    #ifdef PARTICLES
  ...
  for (iwait = 0; iwait < wait_max; iwait++) {
    ...
    // depending on which face arrived, load the buffer into the ghost grid
    Load_NTtransfer_and_Request_Receive_Particles_Transfer(
        status.MPI_TAG, &ireq_particles_transfer);
  }
}

// Load the Number of particles that will be received (Particles.n_recv) and
// make the MPI_Irecv request for that buffer size
void Grid3D::Load_NTtransfer_and_Request_Receive_Particles_Transfer(
    int index, int *ireq_particles_transfer)
{
  ...
  if (index == 0) {
    ...
    if (buffer_length > Particles.G.recv_buffer_size_x0) {
      ...
      Extend_GPU_Array(&recv_buffer_x0_particles,
      ...
      #else
    Check_and_Grow_Particles_Buffer(&recv_buffer_x0_particles,
    ...
    MPI_Irecv(recv_buffer_x0_particles, buffer_length, MPI_CHREAL, source[0], 0,
              world,
  ...
  if (index == 1) {
    ...
    if (buffer_length > Particles.G.recv_buffer_size_x1) {
      printf("Extending Particles Transfer Buffer  ");
      Extend_GPU_Array(&recv_buffer_x1_particles,
                       Particles.G.recv_buffer_size_x1,
                       Particles.G.gpu_allocation_factor * buffer_length, true);
      Particles.G.recv_buffer_size_x1 =
          (part_int_t)Particles.G.gpu_allocation_factor * buffer_length;
    }
      #else
    Check_and_Grow_Particles_Buffer(&recv_buffer_x1_particles,
    ...
    // <<  source[1] <<  std::endl;
    MPI_Irecv(recv_buffer_x1_particles, buffer_length, MPI_CHREAL, source[1], 1,
              world,
              &recv_request_particles_transfer[*ireq_particles_transfer]);
  }
  if (index == 2) {
    ...
    if (buffer_length > Particles.G.recv_buffer_size_y0) {
      printf("Extending Particles Transfer Buffer  ");
      Extend_GPU_Array(&recv_buffer_y0_particles,
                       Particles.G.recv_buffer_size_y0,
                       Particles.G.gpu_allocation_factor * buffer_length, true);
      Particles.G.recv_buffer_size_y0 =
          (part_int_t)Particles.G.gpu_allocation_factor * buffer_length;
    }
    ...
    MPI_Irecv(recv_buffer_y0_particles, buffer_length, MPI_CHREAL, source[2], 2,
              world,
              &recv_request_particles_transfer[*ireq_particles_transfer]);
  }
  if (index == 3) {
    ...
    if (buffer_length > Particles.G.recv_buffer_size_y1) {
      printf("Extending Particles Transfer Buffer  ");
      Extend_GPU_Array(&recv_buffer_y1_particles,
                       Particles.G.recv_buffer_size_y1,
                       Particles.G.gpu_allocation_factor * buffer_length, true);
      Particles.G.recv_buffer_size_y1 =
          (part_int_t)Particles.G.gpu_allocation_factor * buffer_length;
    }
      #else
    Check_and_Grow_Particles_Buffer(&recv_buffer_y1_particles,
    ...
    MPI_Irecv(recv_buffer_y1_particles, buffer_length, MPI_CHREAL, source[3], 3,
              world,
              &recv_request_particles_transfer[*ireq_particles_transfer]);
  }
  if (index == 4) {
    ...
    if (buffer_length > Particles.G.recv_buffer_size_z0) {
      printf("Extending Particles Transfer Buffer  ");
      Extend_GPU_Array(&recv_buffer_z0_particles,
                       Particles.G.recv_buffer_size_z0,
                       Particles.G.gpu_allocation_factor * buffer_length, true);
      Particles.G.recv_buffer_size_z0 =
          (part_int_t)Particles.G.gpu_allocation_factor * buffer_length;
    }
      #else
    Check_and_Grow_Particles_Buffer(&recv_buffer_z0_particles,
    ...
    MPI_Irecv(recv_buffer_z0_particles, buffer_length, MPI_CHREAL, source[4], 4,
              world,
              &recv_request_particles_transfer[*ireq_particles_transfer]);
  }
  if (index == 5) {
    ...
    if (buffer_length > Particles.G.recv_buffer_size_z1) {
      printf("Extending Particles Transfer Buffer  ");
      Extend_GPU_Array(&recv_buffer_z1_particles,
                       Particles.G.recv_buffer_size_z1,
                       Particles.G.gpu_allocation_factor * buffer_length, true);
      Particles.G.recv_buffer_size_z1 =
          (part_int_t)Particles.G.gpu_allocation_factor * buffer_length;
    }
    ...
    MPI_Irecv(recv_buffer_z1_particles, buffer_length, MPI_CHREAL, source[5], 5,
              world,
              &recv_request_particles_transfer[*ireq_particles_transfer]);
  }
...
void Grid3D::Load_and_Send_Particles_X0(int ireq_n_particles,
                                        int ireq_particles_transfer)
{
  ...
  MPI_Irecv(&Particles.n_recv_x0, 1, MPI_PART_INT, source[0], 0, world,
            &recv_request_n_particles[ireq_n_particles]);
  MPI_Isend(&Particles.n_send_x0, 1, MPI_PART_INT, dest[0], 1, world,
            &send_request_n_particles[0]);
  ...
  Check_and_Grow_Particles_Buffer(&send_buffer_x0_particles,
                                  &buffer_length_particles_x0_send,
                                  buffer_length);
  Particles.Load_Particles_to_Buffer_CPU(0, 0, send_buffer_x0_particles,
                                         buffer_length_particles_x0_send);
    #endif  // PARTICLES_CPU
  ...
  MPI_Isend(send_buffer_x0_particles, buffer_length, MPI_CHREAL, dest[0], 1,
            world, &send_request_particles_transfer[ireq_particles_transfer]);
  MPI_Request_free(send_request_particles_transfer + ireq_particles_transfer);
}

void Grid3D::Load_and_Send_Particles_X1(int ireq_n_particles,
                                        int ireq_particles_transfer)
{
  int buffer_length;
  ...
  Particles.Load_Particles_to_Buffer_GPU(0, 1, send_buffer_x1_particles,
                                         buffer_length_particles_x1_send);
    #endif  // PARTICLES_GPU

  MPI_Irecv(&Particles.n_recv_x1, 1, MPI_PART_INT, source[1], 1, world,
            &recv_request_n_particles[ireq_n_particles]);
  MPI_Isend(&Particles.n_send_x1, 1, MPI_PART_INT, dest[1], 0, world,
            &send_request_n_particles[1]);
  ...
  Check_and_Grow_Particles_Buffer(&send_buffer_x1_particles,
                                  &buffer_length_particles_x1_send,
                                  buffer_length);
  Particles.Load_Particles_to_Buffer_CPU(0, 1, send_buffer_x1_particles,
                                         buffer_length_particles_x1_send);
    #endif  // PARTICLES_CPU
  ...
  MPI_Isend(send_buffer_x1_particles, buffer_length, MPI_CHREAL, dest[1], 0,
            world, &send_request_particles_transfer[ireq_particles_transfer]);
  MPI_Request_free(send_request_particles_transfer + ireq_particles_transfer);
}

void Grid3D::Load_and_Send_Particles_Y0(int ireq_n_particles,
                                        int ireq_particles_transfer)
{
  int buffer_length;
  ...
  Particles.Load_Particles_to_Buffer_GPU(1, 0, send_buffer_y0_particles,
                                         buffer_length_particles_y0_send);
    #endif  // PARTICLES_GPU

  MPI_Isend(&Particles.n_send_y0, 1, MPI_PART_INT, dest[2], 3, world,
            &send_request_n_particles[0]);
  MPI_Request_free(send_request_n_particles);
  MPI_Irecv(&Particles.n_recv_y0, 1, MPI_PART_INT, source[2], 2, world,
            &recv_request_n_particles[ireq_n_particles]);
  ...
  Check_and_Grow_Particles_Buffer(&send_buffer_y0_particles,
                                  &buffer_length_particles_y0_send,
                                  buffer_length);
  Particles.Load_Particles_to_Buffer_CPU(1, 0, send_buffer_y0_particles,
                                         buffer_length_particles_y0_send);
    #endif  // PARTICLES_CPU
  ...
  MPI_Isend(send_buffer_y0_particles, buffer_length, MPI_CHREAL, dest[2], 3,
            world, &send_request_particles_transfer[ireq_particles_transfer]);
  MPI_Request_free(send_request_particles_transfer + ireq_particles_transfer);
}

void Grid3D::Load_and_Send_Particles_Y1(int ireq_n_particles,
                                        int ireq_particles_transfer)
{
  int buffer_length;
  ...
  MPI_Isend(&Particles.n_send_y1, 1, MPI_PART_INT, dest[3], 2, world,
            &send_request_n_particles[1]);
  MPI_Request_free(send_request_n_particles + 1);
  MPI_Irecv(&Particles.n_recv_y1, 1, MPI_PART_INT, source[3], 3, world,
            &recv_request_n_particles[ireq_n_particles]);
  ...
  Check_and_Grow_Particles_Buffer(&send_buffer_y1_particles,
                                  &buffer_length_particles_y1_send,
                                  buffer_length);
  Particles.Load_Particles_to_Buffer_CPU(1, 1, send_buffer_y1_particles,
                                         buffer_length_particles_y1_send);
    #endif  // PARTICLES_CPU
  ...
  MPI_Isend(send_buffer_y1_particles, buffer_length, MPI_CHREAL, dest[3], 2,
            world, &send_request_particles_transfer[ireq_particles_transfer]);
  MPI_Request_free(send_request_particles_transfer + ireq_particles_transfer);
}

void Grid3D::Load_and_Send_Particles_Z0(int ireq_n_particles,
                                        int ireq_particles_transfer)
{
  int buffer_length;
  ...
  Particles.Load_Particles_to_Buffer_GPU(2, 0, send_buffer_z0_particles,
                                         buffer_length_particles_z0_send);
    #endif  // PARTICLES_GPU

  MPI_Isend(&Particles.n_send_z0, 1, MPI_PART_INT, dest[4], 5, world,
            &send_request_n_particles[0]);
  MPI_Request_free(send_request_n_particles);
  MPI_Irecv(&Particles.n_recv_z0, 1, MPI_PART_INT, source[4], 4, world,
            &recv_request_n_particles[ireq_n_particles]);
  ...
  Check_and_Grow_Particles_Buffer(&send_buffer_z0_particles,
                                  &buffer_length_particles_z0_send,
                                  buffer_length);
  Particles.Load_Particles_to_Buffer_CPU(2, 0, send_buffer_z0_particles,
                                         buffer_length_particles_z0_send);
    #endif  // PARTICLES_CPU
  ...
  MPI_Isend(send_buffer_z0_particles, buffer_length, MPI_CHREAL, dest[4], 5,
            world, &send_request_particles_transfer[ireq_particles_transfer]);
  MPI_Request_free(send_request_particles_transfer + ireq_particles_transfer);
}

void Grid3D::Load_and_Send_Particles_Z1(int ireq_n_particles,
                                        int ireq_particles_transfer)
{
  int buffer_length;
  ...
  Particles.Load_Particles_to_Buffer_GPU(2, 1, send_buffer_z1_particles,
                                         buffer_length_particles_z1_send);
    #endif  // PARTICLES_GPU

  MPI_Isend(&Particles.n_send_z1, 1, MPI_PART_INT, dest[5], 4, world,
            &send_request_n_particles[1]);
  MPI_Request_free(send_request_n_particles + 1);
  MPI_Irecv(&Particles.n_recv_z1, 1, MPI_PART_INT, source[5], 5, world,
            &recv_request_n_particles[ireq_n_particles]);
  ...
  Check_and_Grow_Particles_Buffer(&send_buffer_z1_particles,
                                  &buffer_length_particles_z1_send,
                                  buffer_length);
  Particles.Load_Particles_to_Buffer_CPU(2, 1, send_buffer_z1_particles,
                                         buffer_length_particles_z1_send);
    #endif  // PARTICLES_CPU
  ...
  MPI_Isend(send_buffer_z1_particles, buffer_length, MPI_CHREAL, dest[5], 4,
            world, &send_request_particles_transfer[ireq_particles_transfer]);
  MPI_Request_free(send_request_particles_transfer + ireq_particles_transfer);
}
...
void Grid3D::Unload_Particles_from_Buffer_X0(int *flags)
{
  ...
  Particles.Unload_Particles_from_Buffer_CPU(
      0, 0, h_recv_buffer_x0_particles, Particles.n_recv_x0,
      h_send_buffer_y0_particles, h_send_buffer_y1_particles,
      h_send_buffer_z0_particles, h_send_buffer_z1_particles,
      buffer_length_particles_y0_send, buffer_length_particles_y1_send,
  ...
  cudaMemcpy(d_recv_buffer_x0_particles, h_recv_buffer_x0_particles,
      ...
      #endif
  Particles.Unload_Particles_from_Buffer_GPU(0, 0, d_recv_buffer_x0_particles,
                                             Particles.n_recv_x0);
    #endif  // PARTICLES_GPU
}
...
void Grid3D::Unload_Particles_from_Buffer_X1(int *flags)
{
    #ifdef PARTICLES_CPU
  Particles.Unload_Particles_from_Buffer_CPU(
      0, 1, h_recv_buffer_x1_particles, Particles.n_recv_x1,
      h_send_buffer_y0_particles, h_send_buffer_y1_particles,
      h_send_buffer_z0_particles, h_send_buffer_z1_particles,
      buffer_length_particles_y0_send, buffer_length_particles_y1_send,
      buffer_length_particles_z0_send, buffer_length_particles_z1_send, flags);
  ...
  cudaMemcpy(d_recv_buffer_x1_particles, h_recv_buffer_x1_particles,
             ...
             cudaMemcpyHostToDevice);
      #endif
  Particles.Unload_Particles_from_Buffer_GPU(0, 1, d_recv_buffer_x1_particles,
                                             Particles.n_recv_x1);
    #endif  // PARTICLES_GPU
}
...
void Grid3D::Unload_Particles_from_Buffer_Y0(int *flags)
{
    #ifdef PARTICLES_CPU
  Particles.Unload_Particles_from_Buffer_CPU(
      1, 0, h_recv_buffer_y0_particles, Particles.n_recv_y0,
      h_send_buffer_y0_particles, h_send_buffer_y1_particles,
      h_send_buffer_z0_particles, h_send_buffer_z1_particles,
      buffer_length_particles_y0_send, buffer_length_particles_y1_send,
      buffer_length_particles_z0_send, buffer_length_particles_z1_send, flags);
  ...
  cudaMemcpy(d_recv_buffer_y0_particles, h_recv_buffer_y0_particles,
             ...
             cudaMemcpyHostToDevice);
      #endif
  Particles.Unload_Particles_from_Buffer_GPU(1, 0, d_recv_buffer_y0_particles,
                                             Particles.n_recv_y0);
    #endif  // PARTICLES_GPU
}
...
void Grid3D::Unload_Particles_from_Buffer_Y1(int *flags)
{
    #ifdef PARTICLES_CPU
  Particles.Unload_Particles_from_Buffer_CPU(
      1, 1, h_recv_buffer_y1_particles, Particles.n_recv_y1,
      h_send_buffer_y0_particles, h_send_buffer_y1_particles,
      h_send_buffer_z0_particles, h_send_buffer_z1_particles,
      buffer_length_particles_y0_send, buffer_length_particles_y1_send,
      buffer_length_particles_z0_send, buffer_length_particles_z1_send, flags);
  ...
  cudaMemcpy(d_recv_buffer_y1_particles, h_recv_buffer_y1_particles,
             ...
             cudaMemcpyHostToDevice);
      #endif
  Particles.Unload_Particles_from_Buffer_GPU(1, 1, d_recv_buffer_y1_particles,
                                             Particles.n_recv_y1);
    #endif  // PARTICLES_GPU
}
...
void Grid3D::Unload_Particles_from_Buffer_Z0(int *flags)
{
    #ifdef PARTICLES_CPU
  Particles.Unload_Particles_from_Buffer_CPU(
      2, 0, h_recv_buffer_z0_particles, Particles.n_recv_z0,
      h_send_buffer_y0_particles, h_send_buffer_y1_particles,
      h_send_buffer_z0_particles, h_send_buffer_z1_particles,
      buffer_length_particles_y0_send, buffer_length_particles_y1_send,
      buffer_length_particles_z0_send, buffer_length_particles_z1_send, flags);
  ...
  cudaMemcpy(d_recv_buffer_z0_particles, h_recv_buffer_z0_particles,
             ...
             cudaMemcpyHostToDevice);
      #endif
  Particles.Unload_Particles_from_Buffer_GPU(2, 0, d_recv_buffer_z0_particles,
                                             Particles.n_recv_z0);
    #endif  // PARTICLES_GPU
}
...
void Grid3D::Unload_Particles_from_Buffer_Z1(int *flags)
{
    #ifdef PARTICLES_CPU
  Particles.Unload_Particles_from_Buffer_CPU(
      2, 1, h_recv_buffer_z1_particles, Particles.n_recv_z1,
      h_send_buffer_y0_particles, h_send_buffer_y1_particles,
      h_send_buffer_z0_particles, h_send_buffer_z1_particles,
      buffer_length_particles_y0_send, buffer_length_particles_y1_send,
      buffer_length_particles_z0_send, buffer_length_particles_z1_send, flags);
  ...
  cudaMemcpy(d_recv_buffer_z1_particles, h_recv_buffer_z1_particles,
             ...
             cudaMemcpyHostToDevice);
      #endif
  Particles.Unload_Particles_from_Buffer_GPU(2, 1, d_recv_buffer_z1_particles,
                                             Particles.n_recv_z1);
    #endif  // PARTICLES_GPU
}
...
int Particles_3D::Select_Particles_to_Transfer_GPU(int direction, int side)
{
  ...
  n_transfer = Select_Particles_to_Transfer_GPU_function(
      n_local, side, domainMin, domainMax, pos, G.n_transfer_d, G.n_transfer_h,
...
void Particles_3D::Copy_Transfer_Particles_to_Buffer_GPU(int n_transfer,
                                                         int direction,
                                                         int side,
                                                         Real *send_buffer_h,
                                                         int buffer_length)
{
  ...
  if ((*n_send + n_transfer) * N_DATA_PER_PARTICLE_TRANSFER > *buffer_size) {
    ...
    Extend_GPU_Array(&send_buffer_d, *buffer_size,
                     G.gpu_allocation_factor * (*n_send + n_transfer) *
                         N_DATA_PER_PARTICLE_TRANSFER,
                     true);
    *buffer_size = (part_int_t)G.gpu_allocation_factor *
                   (*n_send + n_transfer) * N_DATA_PER_PARTICLE_TRANSFER;
  }

  // Load the particles that will be transferred into the buffers
  n_fields_to_transfer = N_DATA_PER_PARTICLE_TRANSFER;
  Load_Particles_to_Transfer_GPU_function(
      n_transfer, ++field_id, n_fields_to_transfer, pos_x_dev,
      G.transfer_particles_indices_d, send_buffer_d, domainMin, domainMax,
      bt_pos_x);
  Load_Particles_to_Transfer_GPU_function(
      n_transfer, ++field_id, n_fields_to_transfer, pos_y_dev,
      G.transfer_particles_indices_d, send_buffer_d, domainMin, domainMax,
      bt_pos_y);
  Load_Particles_to_Transfer_GPU_function(
      n_transfer, ++field_id, n_fields_to_transfer, pos_z_dev,
      G.transfer_particles_indices_d, send_buffer_d, domainMin, domainMax,
      bt_pos_z);
  Load_Particles_to_Transfer_GPU_function(
      n_transfer, ++field_id, n_fields_to_transfer, vel_x_dev,
      G.transfer_particles_indices_d, send_buffer_d, domainMin, domainMax,
      bt_non_pos);
  Load_Particles_to_Transfer_GPU_function(
      n_transfer, ++field_id, n_fields_to_transfer, vel_y_dev,
      G.transfer_particles_indices_d, send_buffer_d, domainMin, domainMax,
      bt_non_pos);
  Load_Particles_to_Transfer_GPU_function(
      n_transfer, ++field_id, n_fields_to_transfer, vel_z_dev,
      G.transfer_particles_indices_d, send_buffer_d, domainMin, domainMax,
      bt_non_pos);
      #ifndef SINGLE_PARTICLE_MASS
  Load_Particles_to_Transfer_GPU_function(
      n_transfer, ++field_id, n_fields_to_transfer, mass_dev,
      G.transfer_particles_indices_d, send_buffer_d, domainMin, domainMax,
      bt_non_pos);
      #endif
      #ifdef PARTICLE_IDS
  Load_Particles_to_Transfer_Int_GPU_function(
      n_transfer, ++field_id, n_fields_to_transfer, partIDs_dev,
      G.transfer_particles_indices_d, send_buffer_d, domainMin, domainMax,
      bt_non_pos);
      #endif
      #ifdef PARTICLE_AGE
  Load_Particles_to_Transfer_GPU_function(
      n_transfer, ++field_id, n_fields_to_transfer, age_dev,
      G.transfer_particles_indices_d, send_buffer_d, domainMin, domainMax,
      bt_non_pos);
      #endif
  CHECK(cudaDeviceSynchronize());
...
void Particles_3D::Replace_Tranfered_Particles_GPU(int n_transfer)
{
  ...
  Replace_Transfered_Particles_GPU_function(
      n_transfer, pos_x_dev, G.transfer_particles_indices_d,
      G.replace_particles_indices_d, false);
  Replace_Transfered_Particles_GPU_function(
      n_transfer, pos_y_dev, G.transfer_particles_indices_d,
      G.replace_particles_indices_d, false);
  Replace_Transfered_Particles_GPU_function(
      n_transfer, pos_z_dev, G.transfer_particles_indices_d,
      G.replace_particles_indices_d, false);
  Replace_Transfered_Particles_GPU_function(
      n_transfer, vel_x_dev, G.transfer_particles_indices_d,
      G.replace_particles_indices_d, false);
  Replace_Transfered_Particles_GPU_function(
      n_transfer, vel_y_dev, G.transfer_particles_indices_d,
      G.replace_particles_indices_d, false);
  Replace_Transfered_Particles_GPU_function(
      n_transfer, vel_z_dev, G.transfer_particles_indices_d,
      G.replace_particles_indices_d, false);
      #ifndef SINGLE_PARTICLE_MASS
  Replace_Transfered_Particles_GPU_function(
      n_transfer, mass_dev, G.transfer_particles_indices_d,
      G.replace_particles_indices_d, false);
      #endif
      #ifdef PARTICLE_IDS
  Replace_Transfered_Particles_Int_GPU_function(
      n_transfer, partIDs_dev, G.transfer_particles_indices_d,
      G.replace_particles_indices_d, false);
      #endif
      #ifdef PARTICLE_AGE
  Replace_Transfered_Particles_GPU_function(
      n_transfer, age_dev, G.transfer_particles_indices_d,
      G.replace_particles_indices_d, false);
      #endif
...
void Particles_3D::Load_Particles_to_Buffer_GPU(int direction, int side,
                                                Real *send_buffer_h,
                                                int buffer_length)
{
  int n_transfer;
  ...
  Copy_Transfer_Particles_to_Buffer_GPU(n_transfer, direction, side,
                                        send_buffer_h, buffer_length);
...
void Particles_3D::Copy_Transfer_Particles_from_Buffer_GPU(int n_recv,
                                                           Real *recv_buffer_d)
{
  int n_fields_to_transfer;
  ...
  int field_id         = -1;
  n_fields_to_transfer = N_DATA_PER_PARTICLE_TRANSFER;
  Unload_Particles_to_Transfer_GPU_function(n_local, n_recv, ++field_id,
                                            n_fields_to_transfer, pos_x_dev,
                                            recv_buffer_d);
  Unload_Particles_to_Transfer_GPU_function(n_local, n_recv, ++field_id,
                                            n_fields_to_transfer, pos_y_dev,
                                            recv_buffer_d);
  Unload_Particles_to_Transfer_GPU_function(n_local, n_recv, ++field_id,
                                            n_fields_to_transfer, pos_z_dev,
                                            recv_buffer_d);
  Unload_Particles_to_Transfer_GPU_function(n_local, n_recv, ++field_id,
                                            n_fields_to_transfer, vel_x_dev,
                                            recv_buffer_d);
  Unload_Particles_to_Transfer_GPU_function(n_local, n_recv, ++field_id,
                                            n_fields_to_transfer, vel_y_dev,
                                            recv_buffer_d);
  Unload_Particles_to_Transfer_GPU_function(n_local, n_recv, ++field_id,
                                            n_fields_to_transfer, vel_z_dev,
                                            recv_buffer_d);
      #ifndef SINGLE_PARTICLE_MASS
  Unload_Particles_to_Transfer_GPU_function(n_local, n_recv, ++field_id,
                                            n_fields_to_transfer, mass_dev,
                                            recv_buffer_d);
      #endif
      #ifdef PARTICLE_IDS
  Unload_Particles_Int_to_Transfer_GPU_function(n_local, n_recv, ++field_id,
                                                n_fields_to_transfer,
                                                partIDs_dev, recv_buffer_d);
      #endif
      #ifdef PARTICLE_AGE
  Unload_Particles_to_Transfer_GPU_function(n_local, n_recv, ++field_id,
                                            n_fields_to_transfer, age_dev,
                                            recv_buffer_d);
      #endif
...
void Particles_3D::Unload_Particles_from_Buffer_GPU(int direction, int side,
                                                    Real *recv_buffer_h,
                                                    int n_recv)
{
  int buffer_size;
