#if defined(SUPERNOVA) && defined(PARTICLES_GPU) && defined(PARTICLE_AGE) && \
 ...
 */
void supernova::initState(struct parameters* P, part_int_t n_local,
...
__device__ Real Calc_Timestep(Real gamma, Real* density, Real* momentum_x,
                              Real* momentum_y, Real* momentum_z, Real* energy,
...
__device__ Real GetAverageDensity(Real* density, int xi, int yi, int zi,
                                  int nxg, int nyg, int ng)
{
  ...
  for (int i = -1; i < 2; i++) {
    for (int j = -1; j < 2; j++) {
      for (int k = -1; k < 2; k++) {
        d_average += density[(xi + ng + i) + (yi + ng + j) * nxg +
                             (zi + ng + k) * nxg * nyg];
      }
    }
  }
...
__device__ Real GetAverageNumberDensity_CGS(Real* density, int xi, int yi,
                                            int zi, int nxg, int nyg, int ng)
{
  return GetAverageDensity(density, xi, yi, zi, nxg, nyg, ng) * DENSITY_UNIT /
         (supernova::MU * MP);
}
...
__global__ void Cluster_Feedback_Kernel(
    ...
    curandStateMRG32k3a_t* states, Real* prev_dens, int* prev_N,
    short direction, Real* dev_snr, Real snr_dt, Real time_sn_start,
    Real time_sn_end)
{
  __shared__ Real
      s_info[FEED_INFO_N *
             TPB_FEEDBACK];  // for collecting SN feedback information, like #
                             // of SNe or # resolved.
  int tid  = threadIdx.x;
  ...
  s_info[FEED_INFO_N * tid + 5] =
      0;  // unresolved KE added via momentum injection
  ...
  if (gtid < n_local) {
    ...
    Real feedback_energy = 0, feedback_density = 0, feedback_momentum = 0, n_0,
         shell_radius;
    ...
    if (!ignore && in_local) {
      int N = 0;
      if ((t - age_dev[gtid]) <=
          time_sn_end) {  // only calculate this if there will be SN feedback
        if (direction == -1)
          N = -prev_N[gtid];
        else {
          curandStateMRG32k3a_t state = states[gtid];
          N                           = curand_poisson(&state,
                                                       GetSNRate(t - age_dev[gtid], dev_snr, snr_dt,
        ...
        if (N != 0) {
          ...
          // int devId;
          // cudaGetDevice(&devId);
          // printf("[%d: %d] N: %d, time: %.4e, dt: %.4e, e: %.4e, n_0:
          // %.4e\n", devId, gtid, N, t, dt, feedback_energy, n_0);

          feedback_momentum = direction * supernova::FINAL_MOMENTUM *
                              pow(n_0, -0.17) * pow(fabsf(N), 0.93) / dV;
          shell_radius =
              supernova::R_SH * pow(n_0, -0.46) * pow(fabsf(N), 0.29);
          is_resolved = 3 * max(dx, max(dy, dz)) <= shell_radius;
          if (!is_resolved)
            printf(
                "UR[%f] at (%d, %d, %d)  id=%d, N=%d, shell_rad=%0.4e, "
                "n_0=%0.4e\n",
                t, indx_x + n_ghost, indx_y + n_ghost, indx_z + n_ghost,
                (int)id[gtid], N, shell_radius, n_0);
          ...
          if (is_resolved) {  // if resolved inject energy and density
            ...
            // printf("(%d):indx:(%d, %d, %d)\n", gtid, indx_x, indx_y, indx_z);
            // printf("(%d): pos:(%.4e, %.4e, %.4e), delta_x (%.2e, %.2e,
            ...
            if (abs(feedback_momentum / feedback_density * VELOCITY_UNIT *
                    1e-5) >
                40000) {  // injected speeds are greater than 4e4 km/s
              printf(
                  "**** (%d, %d, %d) injected speeds are %.3e km/s\n", indx_x,
                  indx_y, indx_z,
...
Real supernova::Cluster_Feedback(Grid3D& G, FeedbackAnalysis& analysis)
{
  ...
  do {
    ...
    if (G.Particles.n_local > 0) {
      hipLaunchKernelGGL(
          ...
          G.Particles.pos_y_dev, G.Particles.pos_z_dev, G.Particles.mass_dev,
          ...
          d_info, G.C.d_density, G.C.d_GasEnergy, G.C.d_Energy,
          G.C.d_momentum_x, G.C.d_momentum_y, G.C.d_momentum_z, gama,
          supernova::randStates, d_prev_dens, d_prev_N, direction, dev_snr,
          snr_dt, time_sn_start, time_sn_end);
    ...
    if (h_dti != 0 &&
        (C_cfl / h_dti <
         G.H.dt)) {  // timestep too big: need to undo the last operation
      direction = -1;
      if (G.Particles.n_local > 0) {
        hipLaunchKernelGGL(
            Cluster_Feedback_Kernel, ngrid, TPB_FEEDBACK, 0, 0,
  ...
  if (info[supernova::RESOLVED] > 0 || info[supernova::NOT_RESOLVED] > 0) {
    resolved_ratio =
        info[supernova::RESOLVED] /
        (info[supernova::RESOLVED] + info[supernova::NOT_RESOLVED]);
  }
  ...
  chprintf(
      ...
      "unres_energy: %.5e erg\n",
      info[supernova::ENERGY] * MASS_UNIT * LENGTH_UNIT * LENGTH_UNIT /
          TIME_UNIT / TIME_UNIT,
      info[supernova::MOMENTUM] * VELOCITY_UNIT / 1e5,
      info[supernova::UNRES_ENERGY] * MASS_UNIT * LENGTH_UNIT * LENGTH_UNIT /
          TIME_UNIT / TIME_UNIT);
  chprintf(
  ...
  chprintf(
      ...
      "energy: %.5e\n",
      analysis.totalEnergy * MASS_UNIT * LENGTH_UNIT * LENGTH_UNIT / TIME_UNIT /
          TIME_UNIT,
      analysis.totalMomentum * VELOCITY_UNIT / 1e5,
      analysis.totalUnresEnergy * MASS_UNIT * LENGTH_UNIT * LENGTH_UNIT /
          TIME_UNIT / TIME_UNIT);
