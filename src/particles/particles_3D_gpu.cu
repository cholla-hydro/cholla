...
void Particles_3D::Allocate_Particles_Grid_Field_Real(Real **array_dev,
                                                      int size)
{
...
void Particles_3D::Print_Max_Memory_Usage()
{
  ...
  chprintf(
      ...
      n_local_max, fraction_max * 100, mem_usage / 1000000,
      global_free_min / 1000000);
}
...
template <typename T>
void __global__ Copy_Device_to_Device_Kernel(T *src_array_dev, T *dst_array_dev,
                                             part_int_t size)
{
...
void Copy_Device_to_Device(T *src_array_dev, T *dst_array_dev, part_int_t size)
{
  ...
  hipLaunchKernelGGL(Copy_Device_to_Device_Kernel, dim1dGrid, dim1dBlock, 0, 0,
                     src_array_dev, dst_array_dev, size);
  CudaCheckError();
}

void Particles_3D::Allocate_Particles_GPU_Array_Real(Real **array_dev,
                                                     part_int_t size)
{
  size_t global_free, global_total;
  CudaSafeCall(cudaMemGetInfo(&global_free, &global_total));
...
void Particles_3D::Allocate_Particles_GPU_Array_int(int **array_dev,
                                                    part_int_t size)
{
  size_t global_free, global_total;
  CudaSafeCall(cudaMemGetInfo(&global_free, &global_total));
...
void Particles_3D::Allocate_Particles_GPU_Array_Part_Int(part_int_t **array_dev,
                                                         part_int_t size)
{
  size_t global_free, global_total;
  CudaSafeCall(cudaMemGetInfo(&global_free, &global_total));
...
void Particles_3D::Allocate_Particles_GPU_Array_bool(bool **array_dev,
                                                     part_int_t size)
{
  size_t global_free, global_total;
  CudaSafeCall(cudaMemGetInfo(&global_free, &global_total));
...
void Particles_3D::Copy_Particles_Array_Real_Host_to_Device(Real *array_host,
                                                            Real *array_dev,
                                                            part_int_t size)
{
  CudaSafeCall(cudaMemcpy(array_dev, array_host, size * sizeof(Real),
                          cudaMemcpyHostToDevice));
  cudaDeviceSynchronize();
}

void Particles_3D::Copy_Particles_Array_Real_Device_to_Host(Real *array_dev,
                                                            Real *array_host,
                                                            part_int_t size)
{
  CudaSafeCall(cudaMemcpy(array_host, array_dev, size * sizeof(Real),
                          cudaMemcpyDeviceToHost));
  cudaDeviceSynchronize();
}

void Particles_3D::Copy_Particles_Array_Int_Host_to_Device(
    part_int_t *array_host, part_int_t *array_dev, part_int_t size)
{
  CudaSafeCall(cudaMemcpy(array_dev, array_host, size * sizeof(part_int_t),
                          cudaMemcpyHostToDevice));
  cudaDeviceSynchronize();
}

void Particles_3D::Copy_Particles_Array_Int_Device_to_Host(
    part_int_t *array_dev, part_int_t *array_host, part_int_t size)
{
  CudaSafeCall(cudaMemcpy(array_host, array_dev, size * sizeof(part_int_t),
                          cudaMemcpyDeviceToHost));
  cudaDeviceSynchronize();
}

__global__ void Set_Particles_Array_Real_Kernel(Real value, Real *array_dev,
                                                part_int_t size)
{
  int tid = blockIdx.x * blockDim.x + threadIdx.x;
...
void Particles_3D::Set_Particles_Array_Real(Real value, Real *array_dev,
                                            part_int_t size)
{
  ...
  dim3 dim1dBlock(TPB_PARTICLES, 1, 1);
  hipLaunchKernelGGL(Set_Particles_Array_Real_Kernel, dim1dGrid, dim1dBlock, 0,
                     0, value, array_dev, size);
  CudaCheckError();
}
