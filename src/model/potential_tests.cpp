...
struct RazorThinExponentialDisk{
  ...
  Real gr_disk_D3D(Real R, Real z) const noexcept
  {
    ...
    Real vrot_sq = 4 * M_PI * GN * this->Sigma0 * this->scale_length * (y * y) * (
...
TEST(tALLPotentialAprroxExponentialDisk3MN, comparison)
{
  ...
  AprroxExponentialDisk3MN approx = AprroxExponentialDisk3MN::create(
    disk_mass, scale_length, scale_height, true);
  ...
  for (int i = 1; i <= 300; i++){ // skip i = 0
    ...
    ASSERT_LE(std::fabs(approx_phi - ref_phi),
              std::fabs(Phi_tol * ref_phi)
