...
class DiskGalaxy
{
 ...
 public:
  DiskGalaxy(Real md, Real rd, Real zd, Real mvir, Real rvir, Real cvir,
             Real rcool)
  {
    M_d    = md;
    R_d    = rd;
  ...
  Real rho_disk_D3D(const Real r, const Real z)
  {
    ...
    return M_d * c * c * (a * (d * d + r * r) + 3.0 * b * d * d) /
           (4.0 * M_PI * b * b * b * pow(s, 2.5));
  }
  ...
  Real kappa2(Real R, Real z)
  {
    ...
    Real phiH_prime = -C * R / (r * r) / (1 + x) +
                      C * log(1 + x) * R_h * R / (r * r * r) +
                      GN * M_d * R / (B * B * B);
    Real phiH_prime_prime =
        -C / (r * r) / (1 + x) + 2 * C * R * R / (r * r * r * r) / (1 + x) +
...
class ClusteredDiskGalaxy : public DiskGalaxy
{
 ...
 public:
  ClusteredDiskGalaxy(Real lm, Real hm, Real md, Real rd, Real zd, Real mvir,
                      Real rvir, Real cvir, Real rcool)
      : DiskGalaxy{md, rd, zd, mvir, rvir, cvir, rcool},
        lower_cluster_mass{lm},
        higher_cluster_mass{hm}
  {
  ...
  std::vector<Real> generateClusterPopulationMasses(int N,
...
static ClusteredDiskGalaxy MW(1e4, 5e5, 6.5e10, 2.7, 0.7, 1.077e12, 261, 18,
                              157.0);
