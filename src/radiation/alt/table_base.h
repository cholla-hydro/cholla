...
template <typename value_t, unsigned int N, bool VarFirst>
struct DEVICE_ALIGN_DECL TableBase {
 ...
 protected:
  ...
  //  Sample the table
  //
  DEVICE_LOCAL_DECL inline bool GetValuesImpl(const value_t x[N], value_t* ys, unsigned int begin,
  ...
  DEVICE_LOCAL_DECL inline void Interpolate(const unsigned int ijk[N], const value_t w0[N], const value_t w1[N],
                                            value_t* ys, unsigned int begin, unsigned int end) const
  {
    for (unsigned int var = begin; var < end; var++) {
      if (N == 1) {
        ys[var - begin] = mYs[Lidx(ijk[0] + 0, var)] * w0[0] + mYs[Lidx(ijk[0] + 1, var)] * w1[0];
      } else if (N == 2) {
        ys[var - begin] = (mYs[Lidx(ijk[0] + 0 + mSize[0] * (ijk[1] + 0), var)] * w0[0] +
                           mYs[Lidx(ijk[0] + 1 + mSize[0] * (ijk[1] + 0), var)] * w1[0]) *
                              w0[1] +
                          (mYs[Lidx(ijk[0] + 0 + mSize[0] * (ijk[1] + 1), var)] * w0[0] +
                           mYs[Lidx(ijk[0] + 1 + mSize[0] * (ijk[1] + 1), var)] * w1[0]) *
                              w1[1];
      } else if (N == 3) {
        ys[var - begin] = ((mYs[Lidx(ijk[0] + 0 + mSize[0] * (ijk[1] + 0 + mSize[1] * (ijk[2] + 0)), var)] * w0[0] +
                            mYs[Lidx(ijk[0] + 1 + mSize[0] * (ijk[1] + 0 + mSize[1] * (ijk[2] + 0)), var)] * w1[0]) *
                               w0[1] +
                           (mYs[Lidx(ijk[0] + 0 + mSize[0] * (ijk[1] + 1 + mSize[1] * (ijk[2] + 0)), var)] * w0[0] +
                            mYs[Lidx(ijk[0] + 1 + mSize[0] * (ijk[1] + 1 + mSize[1] * (ijk[2] + 0)), var)] * w1[0]) *
                               w1[1]) *
                              w0[2] +
                          ((mYs[Lidx(ijk[0] + 0 + mSize[0] * (ijk[1] + 0 + mSize[1] * (ijk[2] + 1)), var)] * w0[0] +
                            mYs[Lidx(ijk[0] + 1 + mSize[0] * (ijk[1] + 0 + mSize[1] * (ijk[2] + 1)), var)] * w1[0]) *
                               w0[1] +
                           (mYs[Lidx(ijk[0] + 0 + mSize[0] * (ijk[1] + 1 + mSize[1] * (ijk[2] + 1)), var)] * w0[0] +
                            mYs[Lidx(ijk[0] + 1 + mSize[0] * (ijk[1] + 1 + mSize[1] * (ijk[2] + 1)), var)] * w1[0]) *
                               w1[1]) *
                              w1[2];
      }
    }
