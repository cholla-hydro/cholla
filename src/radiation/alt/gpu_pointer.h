...
class HostBuffer
{
 public:
  ...
  //
  void BlockingTransferToDevice(DeviceBuffer<T>& dest, size_t count = 0,
                                const std::initializer_list<size_t>& offsets = {0, 0}) const;
  void BlockingTransferFromDevice(const DeviceBuffer<T>& src, size_t count = 0,
                                  const std::initializer_list<size_t>& offsets = {0, 0});
...
class TransferBuffers
{
 public:
  ...
  //
  inline void BlockingTransferToDevice(size_t count = 0, const std::initializer_list<size_t>& offsets = {0, 0})
  {
    this->hBuffer.BlockingTransferToDevice(this->dBuffer, count, offsets);
  }
  inline void BlockingTransferFromDevice(size_t count = 0, const std::initializer_list<size_t>& offsets = {0, 0})
  {
    this->hBuffer.BlockingTransferFromDevice(this->dBuffer, count, offsets);
  }
...
//
template <typename T>
inline void HostBuffer<T>::Alloc(size_t count)
{
  if (count != 0) {
    mCount = count;
...
template <typename T>
inline void HostBuffer<T>::Zero()
{
  memset(mPtr, 0, sizeof(T) * mCount);
}

template <typename T>
inline void HostBuffer<T>::BlockingTransferToDevice(DeviceBuffer<T>& dest, size_t count,
...
template <typename T>
inline void HostBuffer<T>::BlockingTransferFromDevice(const DeviceBuffer<T>& src, size_t count,
...
template <typename T>
inline void DeviceBuffer<T>::Alloc(size_t count, bool zero)
{
  if (count != 0) {
    mCount = count;
    GPU_Error_Check(cudaMalloc(&mPtr, count * sizeof(T)));
    if (zero) GPU_Error_Check(cudaMemset(mPtr, 0, sizeof(T) * mCount));
  }
}

template <typename T>
inline void DeviceBuffer<T>::Free()
{
  if (mPtr != nullptr) {
    cudaFree(mPtr);
    mPtr = nullptr;
...
template <typename T>
inline void DeviceBuffer<T>::BlockingZero()
{
  cudaMemset(mPtr, 0, mCount * sizeof(T));
}

template <typename T>
inline void TransferBuffers<T>::Alloc(size_t count)
{
...
template <typename T>
inline void TransferBuffers<T>::Free()
{
  this->hBuffer.Free();
  this->dBuffer.Free();
}
