...
void Physics::AtomicData::Create()
{
  ...
  gCrossSections.csHIatHI =
      gCrossSections.cs[CrossSection::IonizationHI][gCrossSections.thresholds[CrossSection::IonizationHI].idx];
  gCrossSections.csHIatHeI =
      gCrossSections.cs[CrossSection::IonizationHI][gCrossSections.thresholds[CrossSection::IonizationHeI].idx];
  gCrossSections.csHIatHeII =
      gCrossSections.cs[CrossSection::IonizationHI][gCrossSections.thresholds[CrossSection::IonizationHeII].idx];
  gCrossSections.csHeIatHeI =
      gCrossSections.cs[CrossSection::IonizationHeI][gCrossSections.thresholds[CrossSection::IonizationHeI].idx];
  gCrossSections.csHeIatHeII =
      gCrossSections.cs[CrossSection::IonizationHeI][gCrossSections.thresholds[CrossSection::IonizationHeII].idx];
  gCrossSections.csHeIIatHeII =
      gCrossSections.cs[CrossSection::IonizationHeII][gCrossSections.thresholds[CrossSection::IonizationHeII].idx];
...
void CrossSectionBuilder(unsigned int num, float* hnu_eV, float** cs)
{
  ...
  for (unsigned int i = 0; i < num; i++) {
    double E = hnu_eV[i];
    cs[CrossSection::IonizationHI][i] =
        (E < Ry_eV ? 0 : csfit(E, 5.475e-14, 4.298e-01, 0.0, 0.0, 0.0, 3.288e+01, 2.963));
    cs[CrossSection::IonizationHeI][i] =
        (E < Ry_eV * TionHeI / TionHI ? 0 : csfit(E, 9.492e-16, 1.361e+01, 4.434e-01, 2.136, 2.039, 1.469, 3.188));
    cs[CrossSection::IonizationHeII][i] =
        (E < Ry_eV * TionHeII / TionHI ? 0 : csfit(E, 1.369e-14, 1.720, 0.0, 0.0, 0.0, 3.288e+01, 2.963));
