...
template <typename value_t, unsigned int N, char Mode>
struct StaticTableMakerGPU;

template <typename value_t, unsigned int N>
struct StaticTableMakerGPU<value_t, N, 'x'> {
  StaticTableGPU<value_t, N, 'x'>* Create(value_t* ys, unsigned int numVars, const unsigned int* size,
