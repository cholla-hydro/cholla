...
template<typename value_t, unsigned int N> class GPU_ALIGN_DECL StaticTableGPU<value_t,N,'x'> : public TableBase<value_t,N,true>
{

public:

    __device__ inline bool GetValues(const value_t x[N], value_t* ys, unsigned int begin, unsigned int end) const { return this->GetValuesImpl(x,ys,begin,end); }

    StaticTableGPU(value_t* ys, unsigned int numVars, const unsigned int size[N], const value_t xmin[N], const value_t xmax[N])
    {
