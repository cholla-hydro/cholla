...
void Chem_GPU::Copy_Float_Array_to_Device(int size, float *array_h,
                                          float *array_d)
{
  CudaSafeCall(cudaMemcpy(array_d, array_h, size * sizeof(float),
...
{
  CudaSafeCall(cudaMemcpy(array_d, array_h, size * sizeof(Real),
                          cudaMemcpyHostToDevice));
  cudaDeviceSynchronize();
}
...
class Thermal_State
{
 public:
  ...
  // Constructor
  __host__ __device__ Thermal_State(Real U_0 = 1, Real d_0 = 1, Real d_HI_0 = 1,
                                    Real d_HII_0 = 0, Real d_HeI_0 = 1,
                                    Real d_HeII_0 = 0, Real d_HeIII_0 = 1,
                                    Real d_e_0 = 0)
      : U(U_0),
        d(d_0),
        d_HI(d_HI_0),
...
__device__ void get_temperature_indx(Real T, Chemistry_Header &Chem_H,
                                     int &temp_indx, Real &delta_T,
                                     Real temp_old, bool print)
{
...
__device__ Real Get_Cooling_Rates(Thermal_State &TS, Chemistry_Header &Chem_H,
                                  Real dens_number_conv, Real current_z,
                                  Real temp_prev, float photo_h_HI,
                                  float photo_h_HeI, float photo_h_HeII,
                                  bool print)
{
  int temp_indx;
  ...
  if (print)
    printf("mu: %f  temp: %f  temp_indx: %d  delta_T: %f  \n", TS.get_MMW(),
           temp, temp_indx, delta_T);
  U_dot = 0.0;

  // Collisional excitation cooling
  Real cool_ceHI, cool_ceHeI, cool_ceHeII;
  cool_ceHI = interpolate_rate(Chem_H.cool_ceHI_d, temp_indx, delta_T) *
              TS.d_HI * TS.d_e;
  cool_ceHeI = interpolate_rate(Chem_H.cool_ceHeI_d, temp_indx, delta_T) *
               TS.d_HeII * TS.d_e * TS.d_e * dens_number_conv / 4.0;
  cool_ceHeII = interpolate_rate(Chem_H.cool_ceHeII_d, temp_indx, delta_T) *
                TS.d_HeII * TS.d_e / 4.0;
  U_dot -= cool_ceHI + cool_ceHeI + cool_ceHeII;

  // Collisional excitation cooling
  Real cool_ciHI, cool_ciHeI, cool_ciHeII, cool_ciHeIS;
  cool_ciHI = interpolate_rate(Chem_H.cool_ciHI_d, temp_indx, delta_T) *
              TS.d_HI * TS.d_e;
  cool_ciHeI = interpolate_rate(Chem_H.cool_ciHeI_d, temp_indx, delta_T) *
               TS.d_HeI * TS.d_e / 4.0;
  cool_ciHeII = interpolate_rate(Chem_H.cool_ciHeII_d, temp_indx, delta_T) *
                TS.d_HeII * TS.d_e / 4.0;
  cool_ciHeIS = interpolate_rate(Chem_H.cool_ciHeIS_d, temp_indx, delta_T) *
                TS.d_HeII * TS.d_e * TS.d_e * dens_number_conv / 4.0;
  U_dot -= cool_ciHI + cool_ciHeI + cool_ciHeII + cool_ciHeIS;

  // Recombination cooling
  Real cool_reHII, cool_reHeII1, cool_reHeII2, cool_reHeIII;
  cool_reHII = interpolate_rate(Chem_H.cool_reHII_d, temp_indx, delta_T) *
               TS.d_HII * TS.d_e;
  cool_reHeII1 = interpolate_rate(Chem_H.cool_reHeII1_d, temp_indx, delta_T) *
                 TS.d_HeII * TS.d_e / 4.0;
  cool_reHeII2 = interpolate_rate(Chem_H.cool_reHeII2_d, temp_indx, delta_T) *
                 TS.d_HeII * TS.d_e / 4.0;
  cool_reHeIII = interpolate_rate(Chem_H.cool_reHeIII_d, temp_indx, delta_T) *
                 TS.d_HeIII * TS.d_e / 4.0;
  U_dot -= cool_reHII + cool_reHeII1 + cool_reHeII2 + cool_reHeIII;

  // Bremsstrahlung cooling
  Real cool_brem;
  cool_brem = interpolate_rate(Chem_H.cool_brem_d, temp_indx, delta_T) *
              (TS.d_HII + TS.d_HeII / 4.0 + TS.d_HeIII) * TS.d_e;
  U_dot -= cool_brem;

  // Compton cooling or heating
  Real cool_compton, temp_cmb;
  temp_cmb     = 2.73 * (1.0 + current_z);
  cool_compton = Chem_H.cool_compton * pow(1.0 + current_z, 4) *
                 (temp - temp_cmb) * TS.d_e / dens_number_conv;
  U_dot -= cool_compton;

  // Phothoheating
  Real photo_heat;
  photo_heat = (photo_h_HI * TS.d_HI +
                0.25 * (photo_h_HeI * TS.d_HeI + photo_h_HeII * TS.d_HeII)) /
               dens_number_conv;
  U_dot += photo_heat;
  ...
  if (print)
    printf("Cooling piHI: %e   rate: %e \n", photo_h_HI,
           photo_h_HI * TS.d_HI / dens_number_conv);
  if (print)
    printf("Cooling piHeI: %e  rate: %e \n", photo_h_HeI,
           photo_h_HeI * TS.d_HeI / dens_number_conv * 0.25);
  if (print)
    printf("Cooling piHeII: %e rate: %e \n", photo_h_HeII,
           photo_h_HeII * TS.d_HeII / dens_number_conv * 0.25);
...
__device__ void Get_Reaction_Rates(Thermal_State &TS, Chemistry_Header &Chem_H,
                                   Real &k_coll_i_HI, Real &k_coll_i_HeI,
                                   Real &k_coll_i_HeII, Real &k_coll_i_HI_HI,
                                   Real &k_coll_i_HI_HeI, Real &k_recomb_HII,
                                   Real &k_recomb_HeII, Real &k_recomb_HeIII,
                                   bool print)
{
  int temp_indx;
  Real temp, delta_T;
  ...
  k_coll_i_HI_HI =
      interpolate_rate(Chem_H.k_coll_i_HI_HI_d, temp_indx, delta_T);
  k_coll_i_HI_HeI =
      interpolate_rate(Chem_H.k_coll_i_HI_HeI_d, temp_indx, delta_T);
  ...
  k_recomb_HeIII =
      interpolate_rate(Chem_H.k_recomb_HeIII_d, temp_indx, delta_T);
...
__device__ int Binary_Search(int N, Real val, float *data, int indx_l,
                             int indx_r)
{
  int n, indx;
  n    = indx_r - indx_l;
...
__device__ Real linear_interpolation(Real delta_x, int indx_l, int indx_r,
                                     float *array)
{
  float v_l, v_r;
  Real v;
...
__device__ void Get_Current_UVB_Rates(Real current_z, Chemistry_Header &Chem_H,
                                      ...
                                      float &photo_i_HeII, float &photo_h_HI,
                                      float &photo_h_HeI, float &photo_h_HeII,
                                      bool print)
{
  ...
  indx_l  = Binary_Search(Chem_H.n_uvb_rates_samples, current_z,
                          Chem_H.uvb_rates_redshift_d, 0,
  ...
  photo_i_HI   = linear_interpolation(delta_x, indx_l, indx_l + 1,
                                      Chem_H.photo_ion_HI_rate_d);
  photo_i_HeI  = linear_interpolation(delta_x, indx_l, indx_l + 1,
                                      Chem_H.photo_ion_HeI_rate_d);
  photo_i_HeII = linear_interpolation(delta_x, indx_l, indx_l + 1,
                                      Chem_H.photo_ion_HeII_rate_d);
  photo_h_HI   = linear_interpolation(delta_x, indx_l, indx_l + 1,
                                      Chem_H.photo_heat_HI_rate_d);
  photo_h_HeI  = linear_interpolation(delta_x, indx_l, indx_l + 1,
                                      Chem_H.photo_heat_HeI_rate_d);
  photo_h_HeII = linear_interpolation(delta_x, indx_l, indx_l + 1,
                                      Chem_H.photo_heat_HeII_rate_d);
}

__device__ Real Get_Chemistry_dt(
    Thermal_State &TS, Chemistry_Header &Chem_H, Real &HI_dot, Real &e_dot,
    Real U_dot, Real k_coll_i_HI, Real k_coll_i_HeI, Real k_coll_i_HeII,
    Real k_coll_i_HI_HI, Real k_coll_i_HI_HeI, Real k_recomb_HII,
    Real k_recomb_HeII, Real k_recomb_HeIII, float photo_i_HI,
    float photo_i_HeI, float photo_i_HeII, int n_iter, Real HI_dot_prev,
    Real e_dot_prev, Real t_chem, Real dt_hydro, bool print)
{
  ...
  HI_dot = k_recomb_HII * TS.d_HII * TS.d_e - k_coll_i_HI * TS.d_HI * TS.d_e -
           k_coll_i_HI_HI * TS.d_HI * TS.d_HI -
           k_coll_i_HI_HeI * TS.d_HI * TS.d_HeI / 4.0 - photo_i_HI * TS.d_HI;

  // Rate of change of electron
  e_dot = k_coll_i_HI * TS.d_HI * TS.d_e +
          k_coll_i_HeI * TS.d_HeI / 4.0 * TS.d_e +
          k_coll_i_HeII * TS.d_HeII / 4.0 * TS.d_e +
          k_coll_i_HI_HI * TS.d_HI * TS.d_HI +
          +k_coll_i_HI_HeI * TS.d_HI * TS.d_HeI / 4.0 -
          k_recomb_HII * TS.d_HII * TS.d_e -
          k_recomb_HeII * TS.d_HeII / 4.0 * TS.d_e -
          k_recomb_HeIII * TS.d_HeIII / 4.0 * TS.d_e + photo_i_HI * TS.d_HI +
  ...
  if (fmin(fabs(k_coll_i_HI * TS.d_HI * TS.d_e),
           fabs(k_recomb_HII * TS.d_HII * TS.d_e)) /
  ...
  if (TS.get_temperature(Chem_H.gamma) < TEMP_FLOOR)
    TS.U = TS.compute_U(TEMP_FLOOR, Chem_H.gamma);
  #endif
  ...
  if (n_iter == Chem_H.max_iter - 1) {
    printf(
        ...
        dt_hydro, t_chem, TS.d, TS.get_temperature(Chem_H.gamma), energy, U_dot,
        fabs(0.1 * TS.d_HI / HI_dot), fabs(0.1 * TS.d_e / e_dot),
        fabs(0.1 * TS.U * TS.d / U_dot));
  }
...
__device__ void Update_Step(Thermal_State &TS, Chemistry_Header &Chem_H,
                            Real dt, Real U_dot, Real k_coll_i_HI,
                            Real k_coll_i_HeI, Real k_coll_i_HeII,
                            Real k_coll_i_HI_HI, Real k_coll_i_HI_HeI,
                            Real k_recomb_HII, Real k_recomb_HeII,
                            Real k_recomb_HeIII, float photo_i_HI,
                            float photo_i_HeI, float photo_i_HeII,
                            Real &HI_dot_prev, Real &e_dot_prev,
                            Real &temp_prev, bool print)
{
  ...
  a_coef = k_coll_i_HI * TS.d_e + k_coll_i_HI_HI * TS.d_HI +
           k_coll_i_HI_HeI * TS.d_HeI / 4.0 + photo_i_HI;
  d_HI_p = (dt * s_coef + TS.d_HI) / (1.0 + dt * a_coef);
  if (print)
    printf("Update HI  s_coef: %e    a_coef: %e   HIp: %e \n", s_coef, a_coef,
           d_HI_p);
  ...
  if (print)
    printf("Update HII  s_coef: %e    a_coef: %e   HIIp: %e \n", s_coef, a_coef,
           d_HII_p);

  // Update electron
  s_coef = k_coll_i_HI_HI * d_HI_p * d_HI_p +
           k_coll_i_HI_HeI * d_HI_p * TS.d_HeI / 4.0 + photo_i_HI * TS.d_HI +
           photo_i_HeI * TS.d_HeI / 4.0 + photo_i_HeII * TS.d_HeII / 4.0;
  a_coef = -k_coll_i_HI * TS.d_HI + k_recomb_HII * TS.d_HII -
           k_coll_i_HeI * TS.d_HeI / 4.0 + k_recomb_HeII * TS.d_HeII / 4.0 -
           k_coll_i_HeII * TS.d_HeII / 4.0 + k_recomb_HeIII * TS.d_HeIII / 4.0;
  d_e_p = (dt * s_coef + TS.d_e) / (1.0 + dt * a_coef);
  if (print)
    printf("Update e  s_coef: %e    a_coef: %e   ep: %e \n", s_coef, a_coef,
           d_e_p);
  ...
  if (print)
    printf("Update HeI  s_coef: %e    a_coef: %e   HeIp: %e \n", s_coef, a_coef,
           d_HeI_p);

  // Update HeII
  s_coef = k_coll_i_HeI * d_HeI_p * TS.d_e +
           k_recomb_HeIII * TS.d_HeIII * TS.d_e + photo_i_HeI * d_HeI_p;
  a_coef   = k_recomb_HeII * TS.d_e + k_coll_i_HeII * TS.d_e + photo_i_HeII;
  d_HeII_p = (dt * s_coef + TS.d_HeII) / (1.0 + dt * a_coef);
  if (print)
    printf("Update HeII  s_coef: %e    a_coef: %e   HeIIp: %e \n", s_coef,
           a_coef, d_HeII_p);
  ...
  if (print)
    printf("Update HeIII  s_coef: %e    a_coef: %e   HeIIIp: %e \n", s_coef,
           a_coef, d_HeIII_p);
...
__global__ void Update_Chemistry_kernel(Real *dev_conserved, int nx, int ny,
                                        int nz, int n_ghost, int n_fields,
                                        Real dt_hydro, Chemistry_Header Chem_H)
{
  ...
  Real k_coll_i_HI, k_coll_i_HeI, k_coll_i_HeII, k_coll_i_HI_HI,
      k_coll_i_HI_HeI;
  ...
  if (xid > n_ghost - 1 && xid < nx - n_ghost && yid > n_ghost - 1 &&
    ...
    Get_Current_UVB_Rates(current_z, Chem_H, photo_i_HI, photo_i_HeI,
                          photo_i_HeII, photo_h_HI, photo_h_HeI, photo_h_HeII,
                          print);
    ...
    while (t_chem < dt_hydro) {
      ...
      U_dot = Get_Cooling_Rates(TS, Chem_H, Chem_H.dens_number_conv, current_z,
                                temp_prev, photo_h_HI, photo_h_HeI,
                                photo_h_HeII, print);

      Get_Reaction_Rates(TS, Chem_H, k_coll_i_HI, k_coll_i_HeI, k_coll_i_HeII,
                         k_coll_i_HI_HI, k_coll_i_HI_HeI, k_recomb_HII,
                         k_recomb_HeII, k_recomb_HeIII, print);

      dt_chem = Get_Chemistry_dt(
          TS, Chem_H, HI_dot, e_dot, U_dot, k_coll_i_HI, k_coll_i_HeI,
          k_coll_i_HeII, k_coll_i_HI_HI, k_coll_i_HI_HeI, k_recomb_HII,
          k_recomb_HeII, k_recomb_HeIII, photo_i_HI, photo_i_HeI, photo_i_HeII,
          n_iter, HI_dot_prev, e_dot_prev, t_chem, dt_hydro, print);

      Update_Step(TS, Chem_H, dt_chem, U_dot, k_coll_i_HI, k_coll_i_HeI,
                  k_coll_i_HeII, k_coll_i_HI_HI, k_coll_i_HI_HeI, k_recomb_HII,
                  k_recomb_HeII, k_recomb_HeIII, photo_i_HI, photo_i_HeI,
                  photo_i_HeII, HI_dot_prev, e_dot_prev, temp_prev, print);
    ...
    correct_He =
        (1.0 - Chem_H.H_fraction) * TS.d / (TS.d_HeI + TS.d_HeII + TS.d_HeIII);
    TS.d_HI *= correct_H;
    TS.d_HII *= correct_H;
    TS.d_HeI *= correct_He;
                         ...
                         Chemistry_Header &Chem_H)
{
  ...
  hipLaunchKernelGGL(Update_Chemistry_kernel, dim1dGrid, dim1dBlock, 0, 0,
                     dev_conserved, nx, ny, nz, n_ghost, n_fields, dt, Chem_H);
  ...
  #ifndef kboltz
    #define kboltz \
      1.3806504e-16  // Boltzmann's constant [cm2gs-2K-1] or [ergK-1]
  #endif
...
__device__ Real coll_i_HI_rate(Real T, Real units)
{
  ...
  Real k1 =
      exp(-32.71396786375 + 13.53655609057 * logT_ev -
          5.739328757388 * pow(logT_ev, 2) + 1.563154982022 * pow(logT_ev, 3) -
          ...
          2.039149852002e-6 * pow(logT_ev, 8)) /
      units;
  if (T_ev <= 0.8) {
    k1 = fmax(tiny, k1);
  }
...
__device__ Real coll_i_HeI_rate(Real T, Real units)
{
  ...
  if (T_ev > 0.8) {
    return exp(-44.09864886561001 + 23.91596563469 * logT_ev -
               ...
               3.649161410833e-6 * pow(logT_ev, 8)) /
           units;
  } else {
    return tiny;
  }
...
__device__ Real recomb_HII_rate(Real T, Real units, bool use_case_B)
{
  if (use_case_B) {
    ...
    if (T > 5500) {
      ...
      return exp(-28.61303380689232 - 0.7241125657826851 * logT_ev -
...
__device__ Real coll_i_HeII_rate(Real T, Real units)
{
  ...
  Real k5;
  if (T_ev > 0.8) {
    k5 = exp(-68.71040990212001 + 43.93347632635 * logT_ev -
             ...
             3.165581065665e-6 * pow(logT_ev, 8)) /
         units;
  } else {
    k5 = tiny;
  }
...
__device__ Real recomb_HeIII_rate_case_A(Real T, Real units)
{
  Real k6;
  // Has case B recombination setting.
  k6 = 3.36e-10 / sqrt(T) / pow(T / 1.0e3, 0.2) / (1.0 + pow(T / 1.0e6, 0.7)) /
       units;
  return k6;
}
// k6_rate Case B
...
{
  return 9.1e-27 * exp(-fmin(log(dhuge), 13179.0 / T)) * pow(T, -0.1687) /
         (1.0 + sqrt(T / 1.0e5)) / units;
}
...
{
  return 5.54e-17 * exp(-fmin(log(dhuge), 473638.0 / T)) * pow(T, -0.3970) /
         (1.0 + sqrt(T / 1.0e5)) / units;
}
...
__host__ __device__ Real cool_reHII_rate(Real T, Real units, bool use_case_B)
{
  ...
  if (use_case_B) {
    return 3.435e-30 * T * pow(lambdaHI, 1.970) /
           pow(1.0 + pow(lambdaHI / 2.25, 0.376), 3.720) / units;
  } else {
    return 1.778e-29 * T * pow(lambdaHI, 1.965) /
           pow(1.0 + pow(lambdaHI / 0.541, 0.502), 2.697) / units;
  }
}
...
__host__ __device__ Real cool_reHeIII_rate(Real T, Real units, bool use_case_B)
{
  ...
  if (use_case_B) {
    return 8.0 * 3.435e-30 * T * pow(lambdaHeIII, 1.970) /
           pow(1.0 + pow(lambdaHeIII / 2.25, 0.376), 3.720) / units;
  } else {
    return 8.0 * 1.778e-29 * T * pow(lambdaHeIII, 1.965) /
           pow(1.0 + pow(lambdaHeIII / 0.541, 0.502), 2.697) / units;
  }
}
...
__host__ __device__ Real cool_brem_rate(Real T, Real units)
{
  return 1.43e-27 * sqrt(T) *
         (1.1 + 0.34 * exp(-pow(5.5 - log10(T), 2) / 3.0)) / units;
}
