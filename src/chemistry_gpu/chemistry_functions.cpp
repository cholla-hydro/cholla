...
void Grid3D::Initialize_Chemistry(struct parameters *P)
{
  ...
  Chem.H.density_units =
      Chem.H.density_units / Chem.H.a_value / Chem.H.a_value / Chem.H.a_value;
  ...
  Chem.H.cooling_units =
      (pow(length_base, 2) * pow(MH, 2)) / (dens_base * pow(time_base, 3));
  ...
  Chem.H.density_conversion = Cosmo.rho_0_gas * Cosmo.cosmo_h * Cosmo.cosmo_h /
                              pow(kpc_cgs, 3) * MSUN_CGS;
  Chem.H.energy_conversion =
      Cosmo.v_0_gas * Cosmo.v_0_gas * 1e10;  // km^2 -> cm^2 ;
  #else                                      // Not COSMOLOGY
...
void Chem_GPU::Generate_Reaction_Rate_Table(Real **rate_table_array_d,
                                            Rate_Function_T rate_function,
                                            Real units)
{
  ...
  Copy_Real_Array_to_Device(H.N_Temp_bins, rate_table_array_h,
                            *rate_table_array_d);
...
void Chem_GPU::Initialize_Cooling_Rates()
{
  ...
  if (!use_case_B_recombination) {
    Generate_Reaction_Rate_Table(&H.cool_reHII_d, cool_reHII_rate_case_A,
                                 units);
    Generate_Reaction_Rate_Table(&H.cool_reHeII1_d, cool_reHeII1_rate_case_A,
                                 units);
    Generate_Reaction_Rate_Table(&H.cool_reHeIII_d, cool_reHeIII_rate_case_A,
                                 units);
  } else {
    Generate_Reaction_Rate_Table(&H.cool_reHII_d, cool_reHII_rate_case_B,
                                 units);
    Generate_Reaction_Rate_Table(&H.cool_reHeII1_d, cool_reHeII1_rate_case_B,
                                 units);
    Generate_Reaction_Rate_Table(&H.cool_reHeIII_d, cool_reHeIII_rate_case_B,
                                 units);
  }
...
void Chem_GPU::Initialize_Reaction_Rates()
{
  ...
  if (!use_case_B_recombination) {
    Generate_Reaction_Rate_Table(&H.k_recomb_HII_d, recomb_HII_rate_case_A,
                                 units);
    Generate_Reaction_Rate_Table(&H.k_recomb_HeII_d, recomb_HeII_rate_case_A,
                                 units);
    Generate_Reaction_Rate_Table(&H.k_recomb_HeIII_d, recomb_HeIII_rate_case_A,
                                 units);
  } else {
    Generate_Reaction_Rate_Table(&H.k_recomb_HII_d, recomb_HII_rate_case_B,
                                 units);
    Generate_Reaction_Rate_Table(&H.k_recomb_HeII_d, recomb_HeII_rate_case_B,
                                 units);
    Generate_Reaction_Rate_Table(&H.k_recomb_HeIII_d, recomb_HeIII_rate_case_B,
                                 units);
  }
}
...
void Chem_GPU::Initialize_UVB_Ionization_and_Heating_Rates(struct parameters *P)
{
  ...
  Bind_GPU_Textures(n_uvb_rates_samples, Heat_rates_HI_h, Heat_rates_HeI_h,
                    Heat_rates_HeII_h, Ion_rates_HI_h, Ion_rates_HeI_h,
                    Ion_rates_HeII_h);
  #endif
}
...
void Chem_GPU::Copy_UVB_Rates_to_GPU()
{
  ...
  Copy_Float_Array_to_Device(n_uvb_rates_samples, Heat_rates_HI_h,
                             Heat_rates_HI_d);
  Copy_Float_Array_to_Device(n_uvb_rates_samples, Heat_rates_HeI_h,
                             Heat_rates_HeI_d);
  Copy_Float_Array_to_Device(n_uvb_rates_samples, Heat_rates_HeII_h,
                             Heat_rates_HeII_d);
  Copy_Float_Array_to_Device(n_uvb_rates_samples, Ion_rates_HI_h,
                             Ion_rates_HI_d);
  Copy_Float_Array_to_Device(n_uvb_rates_samples, Ion_rates_HeI_h,
                             Ion_rates_HeI_d);
  Copy_Float_Array_to_Device(n_uvb_rates_samples, Ion_rates_HeII_h,
                             Ion_rates_HeII_d);
}
...
void Grid3D::Update_Chemistry()
{
  ...
  Do_Chemistry_Update(C.device, H.nx, H.ny, H.nz, H.n_ghost, H.n_fields, H.dt,
                      Chem.H);
}

void Grid3D::Compute_Gas_Temperature(Real *temperature,
                                     bool convert_cosmo_units)
{
  int k, j, i, id;
    ...
    for (j = 0; j < H.ny; j++) {
      for (i = 0; i < H.nx; i++) {
        ...
        GE = (E -
              0.5 * d *
                  (vx * vx + vy * vy +
                   vz * vz));  // TODO: this probably needs to be fixed for MHD
  #endif
        ...
        cell_n = dens_HI + dens_HII + (dens_HeI + dens_HeII + dens_HeIII) / 4 +
                 dens_e;
        mu = cell_dens / cell_n;
