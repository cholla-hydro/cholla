...
def destination_safe_open(filename: pathlib.Path) -> h5py.File:
    ...
    try:
        ...
        raise FileExistsError(
            f'File "{filename}" already exists and will not be overwritten, skipping.'
        )
...
def _add_snaps_arg(cli, required: bool = False):
    cli.add_argument(
        "--snaps",
        type=_integer_sequence,
        dest="concat_outputs",
        required=required,
        metavar="(NUM | START:STOP[:STEP] | N1,N2,...)",
        help=(
            "Specify output(s) to concatenate. Either a single number "
...
def add_src_cli_args(cli: argparse.ArgumentParser, add_concat_outputs_arg: bool = True):
    ...
    def concat_output(
        ...
        if not set(cleaned_argument).issubset(allowed_charaters):
            raise ValueError(
                "Argument contains incorrect characters. Should only contain '0-9', ',', and '-'."
            )
        ...
        for arg in cleaned_argument:
            if "-" not in arg:
                ...
                if end < start:
                    raise ValueError(
                        "The end of a range must be larger than the start of the range."
                    )
                if start < 0:
                    raise ValueError()
...
def add_common_cli_args(cli: argparse.ArgumentParser, num_processes_choice: str):
    ...
    cli.add_argument(
        ...
        type=pathlib.Path,
        required=True,
        help="The path to the directory to write out the concatenated HDF5 files.",
    )
    ...
    cli.add_argument(
        ...
        type=skip_fields,
        default=[],
        help="List of fields to skip concatenating. Defaults to empty.",
    )
    cli.add_argument(
        "--dtype",
        type=str,
        default=None,
        help="The data type of the output datasets. Accepts most numpy types. Defaults to the same as the input datasets.",
    )
    cli.add_argument(
        "--compression-type",
        type=str,
        default=None,
        help="What kind of compression to use on the output data. Defaults to None.",
    )
    cli.add_argument(
        "--compression-opts",
        type=str,
        default=None,
        help="What compression settings to use if compressing. Defaults to None.",
    )
    cli.add_argument(
        "--chunking",
        type=chunk_arg,
        ...
        nargs="?",
        const=True,
        help="Enable chunking of the output file. Default is `False`. If set without an argument then the chunk size will be automatically chosen or a tuple can be passed to indicate the chunk size desired.",
    )
...
def get_source_path_builder(
    source_directory: pathlib.Path, pre_extension_suffix: str, known_output_snap: int
):
    """
    ...
    new_style_path = _get_source_path(
        proc_id=0, nfile=known_output_snap, new_style=True, **common_kw
    )
    old_style_path = _get_source_path(
        proc_id=0, nfile=known_output_snap, new_style=False, **common_kw
    )
