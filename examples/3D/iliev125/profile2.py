...
fig.subplots_adjust(left=0.10,right=0.98,bottom=0.11,top=0.98,hspace=0,wspace=0.1)

axx = fig.subplots(2,3,sharex='all',sharey='row')


axx[1][0].set_xlim(0,1.05)
axx[1][1].set_xlim(0,1.05)
axx[1][2].set_xlim(0,1.05)
axx[1][0].set_xlabel(r"$r/L_{\rm box}$")
axx[1][1].set_xlabel(r"$r/L_{\rm box}$")
axx[1][2].set_xlabel(r"$r/L_{\rm box}$")
...
def PlotC1(axx,t,fnames,color="orange",lbox=2,alpha=0,eps=0.01,lw=2,labs=False):
    ...
    axx[1].plot(r,np.log10(1.0e-30+tavg),color=color,linestyle="-",label=r"$t=%d{\rm Myr}$"%t if(labs) else None)
##    
...
def PlotA1(axx,t,fnames,color,lbox=2,alpha=0,eps=0.001,lw=2,labs=False):
    ...
    for fname in fnames:
        ...
        print("ALTAIT: no file for time=%g"%t)
        return
    ##
    ...
    axx[0].plot(r,np.log10(1.0e-30+vavg),color=color,linestyle="-",linewidth=lw)
    axx[0].plot(r,np.log10(1.0e-30+np.abs(1-vavg)),color=color,linestyle=":",linewidth=lw)
    if(alpha > 0): axx[0].fill_between(r,np.log10(1.0e-10+np.abs(vavg-vrms)),np.log10(1.0e-10+vavg+vrms),color=color,alpha=alpha)
    axx[1].plot(r,np.log10(1.0e-30+tavg),color=color,linestyle="-",linewidth=lw,label=(r"$t=%d{\rm Myr}$"%t if(labs) else None))

    re = 5.4/6.6*(1-np.exp(-t/122.4))**0.333
...
def Plot2(dname,color,lbox=2,eps=0.01,times=[10,100,500],lw=2,labs=False):
    ...
    Fun([axx[0][0],axx[1][0]],times[0],ff,color,eps=eps,lw=lw,labs=labs)
    Fun([axx[0][1],axx[1][1]],times[1],ff,color,eps=eps,lw=lw,labs=labs)
    Fun([axx[0][2],axx[1][2]],times[2],ff,color,eps=eps,lw=lw,labs=labs)
##
