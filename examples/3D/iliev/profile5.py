...
fig.subplots_adjust(
    left=0.10, right=0.98, bottom=0.07, top=0.98, hspace=0.05, wspace=0.08
)
...
def PlotC1(axx, t, fnames, color="orange", lbox=2, alpha=0, eps=0.01, lw=2, labs=False):
    ...
    if alpha > 0:
        axx[0].fill_between(
            r,
            np.log10(1.0e-10 + np.abs(vavg - vrms)),
            np.log10(1.0e-10 + vavg + vrms),
            color=color,
            alpha=alpha,
        )
...
def Plot2(dname, color, lbox=2, times=[10, 200, 500]):
    ...
    Fun([axx[0][0], axx[1][0], axx[2][0]], times[0], ff, color, lbox=lbox)
    Fun([axx[0][1], axx[1][1], axx[2][1]], times[1], ff, color, lbox=lbox)
    Fun([axx[0][2], axx[1][2], axx[2][2]], times[2], ff, color, lbox=lbox)
