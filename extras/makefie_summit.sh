...
OUTPUT_SINGLE_IONIZATION = -DOUTPUT_SINGLE_IONIZATION
OUTPUT_METALS = -DOUTPUT_METALS
SCALAR = -DSCALAR
N_OMP_THREADS_GRACKLE = -DN_OMP_THREADS_GRACKLE=7
...
FLAGS_HYDRO = $(CUDA) $(PRECISION) $(OUTPUT) $(RECONSTRUCTION) $(SOLVER) $(INTEGRATOR) $(DUAL_ENERGY) $(COOLING) $(DYNAMIC_GPU_ALLOC) $(CPU_TIME) $(PRINT_INITIAL_STATS) $(DENSITY_FLOOR) $(TEMPERATURE_FLOOR) $(SCALAR) $(AVERAGE_SLOW_CELLS) $(OUTPUT_ALWAYS) $(TILED_INITIAL_CONDITIONS) $(N_STEPS_LIMIT) 
...
FLAGS_COOLING = $(COOLING) $(GRACKLE_PRECISION) $(OUTPUT_TEMPERATURE) $(OUTPUT_CHEMISTRY) $(OUTPUT_ELECTRONS) $(OUTPUT_METALS) $(OUTPUT_SINGLE_IONIZATION) $(N_OMP_THREADS_GRACKLE)
