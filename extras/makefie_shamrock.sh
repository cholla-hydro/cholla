...
# N_STEPS_LIMIT = -DN_STEPS_LIMIT=5
...
CUFFT_INCL = -I/usr/local/cuda-10.1/targets/x86_64-linux/include
CUFFT_LIBS = -L/usr/local/cuda-10.1/targets/x86_64-linux/lib/ -lcufft
INCL += $(CUFFT_INCL) 
LIBS += $(CUFFT_LIBS) 
endif
...
FLAGS_HYDRO = $(CUDA) $(PRECISION) $(OUTPUT) $(RECONSTRUCTION) $(SOLVER) $(INTEGRATOR) $(DUAL_ENERGY) $(COOLING) $(DYNAMIC_GPU_ALLOC) $(CPU_TIME) $(PRINT_INITIAL_STATS) $(DENSITY_FLOOR) $(TEMPERATURE_FLOOR) $(SCALAR) $(AVERAGE_SLOW_CELLS) $(OUTPUT_ALWAYS) $(TILED_INITIAL_CONDITIONS) $(N_STEPS_LIMIT) $(OUTPUT_DUAL_ENERGY_FLAGS)
