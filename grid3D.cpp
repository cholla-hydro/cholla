 ...
 *  \brief Write the conserved quantities to a file. */
void Grid3D::Write_Grid(FILE *fp)
{
  int id, i, j, k;
  ...
  #ifdef WITH_GHOST
  fwrite(&(C.density[0]), sizeof(Real), H.n_cells, fp);
  fwrite(&(C.momentum_x[0]), sizeof(Real), H.n_cells, fp);
  fwrite(&(C.momentum_y[0]), sizeof(Real), H.n_cells, fp);
  fwrite(&(C.momentum_z[0]), sizeof(Real), H.n_cells, fp);
  fwrite(&(C.Energy[0]), sizeof(Real), H.n_cells, fp);
  #ifdef DE
  fwrite(&(C.GasEnergy[0]), sizeof(Real), H.n_cells, fp);
  #endif //DE
  #endif //WITH_GHOST
